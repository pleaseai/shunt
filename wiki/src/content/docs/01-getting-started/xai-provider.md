...
- **Bearer-leak guard:** a provider with `auth = "xai_oauth"` must keep its `base_url` on an `x.ai` host; anything else fails validation at boot, so the subscription bearer can never be sent off-origin [src/config.rs:424-435](https://github.com/chatbot-pf/shunt/blob/main/src/config.rs#L424-L435).

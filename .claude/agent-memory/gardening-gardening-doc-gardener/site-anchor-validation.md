...
missing translation**. A locale route with no locale source file still resolves and still carries
the *English* heading ids.
...
2. A locale route missing its source file is **not** a broken link — it renders the English
   fallback, so an English fragment on it is *correct*. Only flag anchors on locale pages that
   have a real translation file.

...
- [admin SPA React port (PR #508, #526)](admin-spa-react-port-pr508.md) — verification method for a pure-port PR: diff every function against the still-in-tree JS original, preserved quirky inconsistencies are not bugs. Blind spot: a helper the port *extracts* has no counterpart to diff — `mutate()` flattened three call sites' deliberate error-path differences. #526 (cutover, deletes the server-rendered dashboard) verified clean.

...
still uses the `daily-` control-plane host only when the parsed `api_endpoint` addresses the
production Code Assist host (https, no port, root path) — not a byte-equal match to the constant,
so `:443` and mis-cased spellings keep onboarding on the `daily-` host. Result: **zero findings** — every doc claim (2 `configuration.md` table-row
...
**exactly** (line 1898) — so a non-loopback `base_url` must carry that host. Note the follow-up
this review missed: validation checks only scheme and host, so a *port*, *path prefix*, query, or
fragment survives it, which is why the string compare this review passed as accurate was in fact
defective (fixed in `1947170` by `addresses_production_backend`, which parses the URL). The
precondition holds for the *host*, not for the whole URL — state it that narrowly. Method: when a diff's code comment makes a claim about what
...
"https://cloudcode-pa.googleapis.com".to_string())`. **Superseded in `1947170`**: that name-keyed
lookup was a credential-egress hole (any table parked under the `antigravity` name, including a
`kind = "responses"` one pointing anywhere, would have received the subscription bearer). It now
goes through `login_base_url`, which honors the slot only when it is `kind = "antigravity"` with
`auth = "antigravity_oauth"` — the shape config validation actually vetted.

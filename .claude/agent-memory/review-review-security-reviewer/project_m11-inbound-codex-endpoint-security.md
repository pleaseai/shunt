...
**GAP (RESOLVED on branch):** `relay_passthrough` forwards upstream response headers via a DENYLIST (`PASSTHROUGH_STRIP_RESPONSE_HEADERS`). An earlier review found `set-cookie` was not stripped, so a Cloudflare (`__cf_bm`/`cf_clearance`) or account/session-affinity cookie the chatgpt.com edge set would be relayed verbatim to the untrusted/multi-tenant inbound client. Fixed: `set-cookie`/`set-cookie2` are now in the denylist (CWE-200/CWE-201) and stripped before relay.

**Note:** client-supplied `x-shunt-inbound-client` IS stripped on the inbound path too — it is in `PASSTHROUGH_STRIP_REQUEST_HEADERS`, matching the main proxy path's spoofing guard (proxy.rs), so it never reaches upstream. The pool sticky key is namespaced with the authenticated client name (`{client}:{session-id}`, see `codex_endpoint::pool_sticky_key`), so one client cannot pin another client's session onto a chosen pool account by replaying its `session-id`.

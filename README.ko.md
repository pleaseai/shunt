...
**선택 기능인 지출 한도 Admin API.** `[server.spend]`를 구성하면 조직 단위 및 사용자 단위 한도를 위한 인증된 CRUD 라우트가 `/v1/organizations/spend_limits` 아래에 등록됩니다. 1단계에서는 한도와 감사 기록을 저장하지만 아직 추론 트래픽에 한도를 강제하지는 않습니다. 이 라우트는 `[server.admin]` 자격 증명으로 인증합니다. `[server.spend]`는 키 자료를 담지 않는 최상위 정책 섹션이므로, 지출 한도를 활성화하는 데 `[server.gateway]` 로그인이 필요하지 않습니다. 상태는 기본적으로 원자적으로 기록되는 비공개 JSON 파일에 저장됩니다. `[server.spend.pricing]`에서는 (앞으로 추가될) 지출 측정기가 요청 비용을 계산할 때 사용할 배수와 업스트림·모델별 요금 재정의를 설정할 수 있습니다. 설정, API 동작, 보류된 기능은 [1단계 가이드](docs/gateway-spend-limits.md)를 참고하세요.

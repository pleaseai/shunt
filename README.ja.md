...
モデル id をひとつ、自分で判断させることもできます。[`[models.router]`](https://shunt.sh/ja/guides/stage-router/) エントリは `type` キーでルーティングアルゴリズムを指定します。`stage_router` は強力なティアと効率的なティアを指定し、会話の直近の **tool-result メタデータ**（プロンプトのテキストではなく `tool_use.name` と `tool_result.is_error`）からターンごとにどちらかを選びます。`auto` は同じルーターをアップストリームのプリセットで動かし、`random` は重み付きのターゲットにトラフィックを振り分けつつ 1 つのセッションを同じ枝に留め、`noop` は空のメッセージを返すのでスモークテストに使えます。`stage_router` にはジャッジモデルを指定する [`[models.router.classifier]`](https://shunt.sh/ja/reference/configuration/#modelsrouterclassifierオプション) テーブルを任意で追加できます。このジャッジはシグナルで決まらなかったターンでのみ呼ばれ、クライアントに提供されることはありません。`judge_*`/`gated_*`/`max_judge_calls` の 6 つのキーが内部呼び出しごとに上限を課します。ターゲットはいずれも通常の公開モデル id なので、それぞれのフェイルオーバーチェーン、プール、アダプターをそのまま引き継ぎます（[Switchyard 統合](https://shunt.sh/ja/guides/switchyard/)）。ルーターを設定しなければ挙動は変わりません。

...
モデル id をひとつ、自分で判断させることもできます。[`[models.router]`](https://shunt.sh/ja/guides/stage-router/) エントリは `type` キーでルーティングアルゴリズムを指定します。`stage_router` は強力なティアと効率的なティアを指定し、会話の直近の **tool-result メタデータ**（プロンプトのテキストではなく `tool_use.name` と `tool_result.is_error`）からターンごとにどちらかを選びます。`auto` は同じルーターをアップストリームのプリセットで動かし、`random` は重み付きのターゲットにトラフィックを振り分けつつ 1 つのセッションを同じ枝に留め、`noop` は空のメッセージを返すのでスモークテストに使えます。ターゲットはいずれも通常の公開モデル id なので、それぞれのフェイルオーバーチェーン、プール、アダプターをそのまま引き継ぎます（[Switchyard 統合](https://shunt.sh/ja/guides/switchyard/)）。どのエントリも [`[models.subagents]`](https://shunt.sh/ja/reference/configuration/#modelssubagentsオプション) オーバーレイを併せて持てます。これは委譲された作業 — `Task` サブエージェント、フックエージェント、ワークフローサブエージェント — を別のターゲットへ送るもので、エージェントタイプごとに指定することもでき（`by_type = { Explore = "claude-haiku-4-5" }`）、親セッション自身の宛先はそのままです。`main`、コンパクション、補助のターンがこれを使うことはありません。ルーターを設定しなければ挙動は変わりません。

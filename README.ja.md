...
| 支出上限 Admin API — 組織単位・ユーザー単位の上限。`[server.spend.pricing]` で、(今後追加される) 支出メーターがリクエストの料金計算に使う乗数とアップストリーム・モデル単位の料金オーバーライドを設定（ステージ 1 は保存のみで、まだ適用しません） | `[server.admin]`（管理者資格情報が必須: `tokens_env`、`tokens_file`、または `write_keys`/`read_keys` エントリ — read 階層は GET のみ処理） + `[server.spend]` | [リファレンス](https://shunt.sh/ja/reference/configuration/#serverspendオプション) |

...
- `docs/` — update the relevant milestone/spec when implementation behavior deviates from it (see Git Workflow); add a new `docs/` note for a substantial new subsystem.
- `site/src/content/docs/` — update the affected guide/reference page when user-facing behavior, config keys, endpoints, or CLI change (`guides/` for how-tos, `reference/` for config/endpoints/CLI/troubleshooting).

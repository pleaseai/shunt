...
- Write code in English, and documentation in English except for the `site/`
  locale trees (see [Documentation](#documentation)).
...
above was considered rather than skipped by default. `README.md`, `docs/`, and
code are English-only. `site/` is the exception: it additionally carries
maintained `ko`/`ja`/`zh-cn` translations under
`site/src/content/docs/<locale>/`, so update the English page and its locale
copies together rather than leaving a page translated in some locales only.

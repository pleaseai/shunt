...
**Opt-in gateway spend-limit Admin API.** Configure `[server.gateway.admin]` to register authenticated CRUD routes under `/v1/organizations/spend_limits` for organization- and user-scoped caps. Stage 1 stores limits and an audit trail but does not enforce the caps on inference traffic yet. Admin keys come from environment-variable indirection, and state persists to an atomic private JSON file by default. See the [stage 1 guide](docs/gateway-spend-limits.md) for setup, API behavior, and deferred features.

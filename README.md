...
**Opt-in Claude apps gateway login.** Configure `[server.gateway]` to let managed Claude Code clients sign in through the OAuth device flow (`forceLoginMethod: "gateway"` + `forceLoginGatewayUrl`) instead of distributing one shared static token. shunt serves OAuth discovery, browser approval, device/refresh grants, HS256 access JWTs, and rotating opaque refresh tokens; the issued bearer authenticates `/v1/messages`, `/v1/messages/count_tokens`, and `/v1/models` and composes with `[server.auth]`. The surface is off by default. M-A stores sessions in memory, so a process restart requires users to sign in again. See the [setup guide](https://shunt-docs.pages.dev/guides/gateway-login/), [configuration reference](https://shunt-docs.pages.dev/reference/configuration/#servergateway-optional), and [M-A design note](docs/gateway-login.md).

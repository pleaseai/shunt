...
**Bounded upstream retry.** Transient upstream failures — `429`/`502`/`503`/`504` and connection errors — are retried with exponential backoff and randomized jitter on a provider's single-credential path, before any bytes reach the client (never mid-stream) and never for other `4xx`. It honors `Retry-After`, is held off `count_tokens`, and is configurable per provider under `[providers.<name>.retry]` (on by default, conservative; set `max_retries = 0` to disable). The `claude_oauth`/`chatgpt_oauth` account pools use their own account-rotation failover instead. See the [configuration reference](https://shunt-docs.pages.dev/reference/configuration/#providersnameretry).

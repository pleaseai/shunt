...
| 支出上限 Admin API —— 组织级和用户级上限,并由 `[server.spend.pricing]` 设置(未来的)支出计量器计算请求费用时使用的乘数,以及按上游和模型指定的费率覆盖(stage 1 只存储,尚未实施) | `[server.admin]`（必须提供管理员凭据: `tokens_env`、`tokens_file` 或一条 `write_keys`/`read_keys` —— read 级别只服务 GET） + `[server.spend]` | [参考](https://shunt.sh/zh-cn/reference/configuration/#serverspend可选) |

...
**入站 Codex 端点。** shunt 也可以反向运行:可选的 `[server.codex_endpoint]` 表会注册一个原始的 OpenAI Responses 透传接口(`/responses`、`/v1/responses`、`/backend-api/codex/responses`),使 **Codex CLI 本身**可以把它的 `base_url` 指向 shunt,并在同一个 ChatGPT/Codex OAuth 账号池上做负载均衡 —— 这是逐字节的中继,而不是上面那条经 Anthropic 转换的路径。它还会把 CLI 的两个分析(analytics)路径当作保护隐私的丢弃接收端,只记录经过净化的事件名计数器,绝不把负载转发到上游。可选的 `[[server.codex_endpoint.routes]]` 表还能开启按模型的路由,让指定的模型 id 改由第三方兼容 Responses 的上游(Z.ai GLM、DeepSeek、Kimi Code、MiniMax、OpenRouter、Vercel AI Gateway、原生 OpenAI)提供服务,而不走 ChatGPT 账号池,并把请求体的 `model` 改写为该路由的上游 id。它默认关闭;没有该表时,这些路由都不会被注册。请参阅[使用方法](https://shunt.dev/zh-cn/guides/inbound-codex-endpoint/)和 [M11 行为规范](docs/m11-inbound-codex-endpoint.md)。
...
**可选的客户端用量端点。** 配置 `[server.usage]` 可注册一个只读的 `GET /usage`,它返回共享账号池配额的**经过净化的聚合**视图 —— 每个窗口的剩余余量、重置时间,以及粗粒度的 `ok`/`degraded`/`exhausted` 状态 —— 使非管理员客户端无需管理界面也能预判限流。它与 `/v1/messages` 使用同一个 `[server.auth]` 客户端令牌进行认证(并且需要该表),并且绝不暴露账号名称、数量、优先级、`disabled` 标志或阈值 —— 完整的账号级细节仍然只保留在仅管理员可访问的 `/admin/pool` 后面。只有没有任何未禁用账户报告某个窗口时,该窗口才是 `null`。Codex 响应中的 `x-codex-*` 头部和可选的 `wham/usage` 轮询会填充已观测到的 5 小时和共享每周窗口;只有未被观测到的窗口才是 `null`。Codex 没有 Fable 范围(`7d_oi`)的信号,但混合提供方池中的其他提供方可以提供聚合 Fable 值。除了池级聚合 `pool` 之外,`providers` 映射还以配置的提供方名称为键,为每个参与池化的提供方给出同样经过净化的聚合,使路由到某一提供方的客户端可以读取该提供方自身的余量和状态,而不是整个池的最优值。默认关闭;没有该表时,该路由不会被注册。请参阅[配置参考](https://shunt.dev/reference/configuration/#serverusage-optional)和 [M12 设计说明](docs/m12-client-usage-endpoint.md)。

...
`state_path = ""` keeps caps and audit records in memory only. Omit `state_path` to use `$HOME/.shunt/gateway-spend.json`; an explicitly configured path is used literally, without shell-style `~` expansion. When shunt cannot resolve a home directory, the default path also becomes memory-only. The state file uses a versioned JSON envelope and an atomic private-file replacement. At restore, shunt logs a warning and carries each malformed cap or cap whose amount, currency, object type, or user id fails validation through subsequent saves as its original JSON value. These carry-through records remain hidden from list, get, and delete operations. This makes a rollback lossless when a newer version writes an additive scope or field that the older version cannot parse. Invalid top-level JSON or an unsupported state version aborts startup so a later mutation cannot overwrite unreadable state. The path is fixed at boot; configuration reloads do not move the process-lifetime store to a different file.

...
- **Out-of-band usage reconciliation.** The Anthropic pool can poll `GET /api/oauth/usage` (`usage_refresh_seconds`). This branch does not add an out-of-band Codex poller, so Codex quota state here refreshes from response headers; the ChatGPT backend nevertheless has a private, undocumented usage API that is reserved for #430 and is not called by M10. Opportunistic re-probing is the interim safety net for non-WebSocket outbound selection and the optional inbound HTTP endpoint. A WebSocket-enabled outbound pool instead relies on the observation-time window bound.

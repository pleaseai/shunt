...
- `429` + `Retry-After`.
- `/v1/models` returns the full Anthropic list shape
  `{"data":[{type,id,display_name,created_at,max_input_tokens,max_tokens,capabilities?}],"has_more","first_id","last_id"}`
  — `capabilities` is relayed when a live upstream entry provides it and omitted from curated and
  builtin entries; envelope parity landed in #213, builtin entries carry the upstream metadata,
  and the cursors hold the first/last entry ids as the real API does.

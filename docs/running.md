...
| `shunt.router.decisions` | Counter | `model`, `algorithm`, `target`, `source` | Routing decisions for every `[models.router]` type, by the router's model id (as matched, so with any `[1m]` hint already stripped), the configured `type`, the model id the decision resolved to, and why. `count_tokens` probes are excluded. |

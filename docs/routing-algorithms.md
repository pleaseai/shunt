...
`prefill_router` landed later, behind its own cargo feature (§5).
`[models.subagents]` is PR 3 (§4). The `shunt.router.judge_calls` metric and the `judges` list on `GET
...
classifier form all make judge calls, and remain later steps in the

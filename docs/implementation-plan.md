...
| **Don't modify bodies (Anthropic path)** | Rewriting/redacting the body breaks the beta-header ↔ body-field pairing → hard `400`. Two mutations are legitimate: changing `model` — including the `[providers.*].classifier_model` remap, which changes `model` on the auto-mode classifier request alone and only ever to another model of the same provider — and restoring the Claude Code identity system block on that same request, which upstream rejects without it (`src/adapters/anthropic/auto_mode_classifier.rs`; the identity repair is scoped to that one request shape on subscription-OAuth credentials, every other request stays byte-for-byte). (The OpenAI path fully rebuilds the body, so this applies to pass-through only.) |

...
| **Don't modify bodies (Anthropic path)** | Rewriting/redacting the body breaks the beta-header ↔ body-field pairing → hard `400`. Two mutations are legitimate: changing `model`, and restoring the Claude Code identity system block on the auto-mode classifier request, which upstream rejects without it (`src/adapters/anthropic/auto_mode_classifier.rs`; scoped to that one request shape on subscription-OAuth credentials, every other request stays byte-for-byte). (The OpenAI path fully rebuilds the body, so this applies to pass-through only.) |

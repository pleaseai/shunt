...
- Without that header, or with it present but empty, shunt uses an independent round-robin counter for each provider. A blank value is not a conversation, and it is normalized to absent inside `select_order_inner` rather than at each header read, so every caller of the pool gets the same treatment (issue #566). Were it left as `Some("")` it would take the sticky branch and hash to one constant slot, collapsing every blank-header client onto a single account.

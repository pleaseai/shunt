...
- The hard threshold defaults to `0.98`. An account is near quota when its 5-hour status is exactly `rejected`, its governing weekly status is exactly `rejected`, its shared 5-hour utilization reaches that window's effective soft threshold, or its governing weekly utilization reaches its effective soft threshold. For Fable models, the governing weekly status is `7d_oi` when that utilization is available; otherwise it is shared `7d`. The aggregate status is used as a compatibility fallback only when no per-window status has been recorded. Since issue #135, the effective soft threshold per window `X` resolves as account `threshold_X` → account `threshold` → `[server.pool].default_threshold_X` → `default_threshold` → `hard_threshold`, capped at `hard_threshold`; without any configuration every window uses `0.98`, the legacy `SWITCH_THRESHOLD`.
...
| 429 with `anthropic-ratelimit-unified-7d_oi-status: rejected` while neither `-5h-status` nor `-7d-status` is `rejected`, on a Fable request | Treat as Fable quota exhaustion, cool the account only for Fable requests for numeric `retry-after` (default 60 seconds, clamped to 1–3600 seconds), and rotate. Non-Fable requests remain eligible to use the account. |

...
- Thinking ↔ reasoning round-trips through a base64url `{id, enc}` payload packed into the anthropic thinking `signature` (`src/model/responses_request.rs:267-281`); signatures shunt did not produce are dropped, never forwarded. The mirror now holds on the way back too: `src/adapters/anthropic/thinking.rs` drops the blocks whose signature shunt minted, so a turn served by a Responses (or Gemini, or Cursor) upstream cannot carry an unverifiable signature into `api.anthropic.com` on the next turn — see `docs/notes/anthropic-thinking-signatures.md`.

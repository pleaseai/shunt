...
availability is decided at compile time: it validates like any other, and on a
build without the `prefill-router` cargo feature — which is every release
binary — the load error names that feature
([`routing-algorithms.md`](routing-algorithms.md) §4).

...
/// Clause 10, concurrently. The budget is a ceiling on calls *made*, so it has
/// to hold when several turns of one session are in flight at once — which is
/// the ordinary shape of a Claude Code session, not an exotic one.
///
/// Deliberately separate from `the_call_budget_is_spent_once_per_session`
/// rather than a phase appended to it: that test's second turn asserts against
/// a session the first turn already pinned, and adding a concurrent phase in
/// front of it would silently change what it is measuring.
///
/// The judge answers slowly, so every turn reaches the gate before any of them
/// has committed a pin. Against a snapshot read — the count copied out of the
/// store early in the request and compared much later, with the judge's round
/// trip in between — all eight read zero, all eight pass, and the mock sees
/// eight calls for a budget of one.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn the_call_budget_is_not_overspent_by_concurrent_turns() {
    if !can_bind_loopback() {
        return;
    }
    const TURNS: usize = 8;
    ...
    let efficient = MockServer::start().await;
    let judge = MockServer::start().await;
    // Every turn is served, whichever way its consultation went: the verdict
    // agrees with the picker's default, so the tier never moves.
    tier_mock(EFFICIENT_UPSTREAM_MODEL, TURNS as u64)
        .mount(&efficient)
        .await;
    tier_mock(CAPABLE_UPSTREAM_MODEL, 0).mount(&capable).await;
    // Slow enough that all eight turns are inside the gate at once, and well
    // inside the 500 ms `judge_timeout_ms` the harness configures.
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(verdict(0.9).set_delay(Duration::from_millis(150)))
        .expect(1)
        .mount(&judge)
        .await;
    let gateway = start_gateway(driven_config(&capable, &efficient, judge.uri())).await;
    ...
    for response in responses {
        assert_eq!(response.status(), StatusCode::OK);
        if response.headers()["x-gateway-route-source"] == "llm-classifier" {
            consulted += 1;
        }
    }
    assert_eq!(
        consulted, 1,
        "exactly one turn wins the single reserved call; the rest fall open"
    );

    let received = judge
        .received_requests()
        .await
        .expect("the judge mock recorded its calls");
    assert_eq!(
        received.len(),
        1,
        "`max_judge_calls = 1` is a ceiling on calls made, not on calls a turn \
         thought were left"
    );

...
| 400 model refusal (see below) | 1 hour, for that model on that account only |
...
Pooled responses identify the account — non-streaming and websocket-streaming responses with
an `x-shunt-account` response header, early-committed HTTP streaming with an `event: account`
SSE frame before the relayed content (the committed `200` headers cannot wait for the winning
account):
...
Use neutral account names on a shared gateway — the header and the frame expose the configured label to every authorized client that receives the response. The final translated-error relay after pool exhaustion omits the attribution.

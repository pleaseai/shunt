...
Pooled responses identify the account — non-streaming with an `x-shunt-account` response
header, streaming with an `event: account` SSE frame before the relayed content (committed
`200` headers cannot wait for the winning account):

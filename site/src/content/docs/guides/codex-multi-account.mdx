...
- **Out-of-band usage reconciliation:** the Anthropic pool can poll an authoritative usage API (`usage_refresh_seconds`). This branch does not poll the ChatGPT backend's private, undocumented usage API, so Codex quota state here refreshes from response headers; an out-of-band Codex poller is reserved for #430.

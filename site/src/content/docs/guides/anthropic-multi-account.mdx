...
- The default switch threshold is `0.98`. An account is near quota when its 5-hour status is `rejected`, its governing weekly status is `rejected`, shared 5-hour utilization reaches its threshold, or the governing weekly utilization reaches its threshold. For Fable models, the governing weekly status is `7d_oi` when that utilization is present; otherwise it is shared `7d`. The aggregate status is a compatibility fallback only when no per-window status has been recorded. Thresholds can be lowered per account (`threshold*` fields above) or pool-wide (see [Tuning selection](#tuning-selection-serverpool)).
...
| 429 with only `anthropic-ratelimit-unified-7d_oi-status: rejected`, on a Fable request | Fable quota exhausted: cooldown applies only to Fable requests using numeric `retry-after` (default 60s, clamped to 1–3600s), then rotate. Non-Fable requests can still use the account. |

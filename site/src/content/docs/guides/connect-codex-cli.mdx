...
rotate, 401 → refresh + retry, 5xx → cool down + rotate). A successful pooled response attributes the account — non-streaming with an
`x-shunt-account: <name>` header, streaming with an `event: account` frame before the relayed SSE.
On a shared gateway with [`[server.auth]`](/reference/configuration/)
...
attribute the account — streaming turns with an `event: account` frame before the relayed SSE
(committed `200` headers cannot wait for the winning account), non-streaming turns with an
`x-shunt-account` header:

...
carry an `x-shunt-account` header — the inbound passthrough waits for the winning upstream before
committing, so the header names the account that actually served the turn:

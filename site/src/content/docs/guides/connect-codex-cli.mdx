...
rotate, 401 → refresh + retry, 5xx → cool down + rotate). A successful pooled response attributes the account — non-streaming and websocket-streaming
responses with an `x-shunt-account: <name>` header, early-committed HTTP streaming with an
`event: account` frame before the relayed SSE. On a shared gateway with [`[server.auth]`](/reference/configuration/)
...
attribute the account — early-committed HTTP streaming with an `event: account` frame before the
relayed SSE (committed `200` headers cannot wait for the winning account), websocket-streaming
and non-streaming turns with an `x-shunt-account` header:
...
- A `200` with `content-type: text/event-stream` and an `event: account` frame naming the pool

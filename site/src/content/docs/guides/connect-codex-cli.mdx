...
rotate, 401 → refresh + retry, 5xx → cool down + rotate). A successful pooled response on this endpoint carries an `x-shunt-account: <name>` header —
streaming included, since the inbound passthrough commits only after the winning upstream
responds. (The outbound Responses pool's early-committed HTTP streaming attributes the account
with an `event: account` frame instead; see the Codex multi-account guide.) On a shared gateway with [`[server.auth]`](/reference/configuration/)
...
Current entitled slugs are `gpt-6-astra` / `-sol` / `-luna`, `gpt-5.6-sol` / `-terra` / `-luna`,
and `gpt-5.5` / `gpt-5.4` / `gpt-5.4-mini` / `gpt-5.2` — older accounts may only have the earlier ones. See
...
A raw Responses request against any of the three routes should relay verbatim and, when a pooled
account wins, carry an `x-shunt-account` header naming it — the inbound passthrough waits for the
winning upstream before committing, so the header names the account that actually served the turn:
...
- A `200` with `content-type: text/event-stream` and an `x-shunt-account:` header naming the pool
  account ⇒ the pool served the request and the SSE is relayed unchanged.

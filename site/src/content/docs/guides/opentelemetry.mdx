...
| :-- | :-- | :-- |
| **Traces** | The per-request `proxy_request` span (and `codex_endpoint_request` on the inbound Codex endpoint), tagged with `gen_ai.request.model`, `shunt.provider`, `http.response.status_code`, and `otel.status_code` (`error` on 4xx/5xx) | Head-based sampling via `sample_ratio`. Low-cardinality; no request/response bodies, headers, or credentials. |

...
| `shunt.router.decisions` | Counter | `model`, `algorithm`, `target`, `source` | Routing decisions for every [`[models.router]`](/reference/configuration/) type, by the router's model id (as matched, so with any `[1m]` hint already stripped), the configured `type`, the model id the decision resolved to, and why it was chosen. `count_tokens` probes are excluded. |

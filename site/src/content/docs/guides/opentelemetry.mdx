...
| :-- | :-- | :-- | :-- |
| `shunt.requests` | Counter | `provider`, `model`, `http.response.status_code`, `caller` | Proxied inference requests. `caller` is `client` for inbound traffic and `router` for a judge call a [`[models.router]`](/reference/configuration/) entry made on the caller's behalf. |
| `shunt.latency` | Histogram (ms) | `provider`, `model`, `http.response.status_code`, `caller` | Header latency for streams; full latency otherwise. The committed streaming responses sample from dispatch to attempt classification, since their headers commit before the upstream responds. |
| `shunt.ttft` | Histogram (ms) | `provider`, `model` | Request start to the first complete non-keepalive SSE frame. |
...
| `shunt.gateway_telemetry_ingest` | Counter | `signal`, `outcome` | Inbound gateway OTLP payloads (`metrics`/`logs`/`traces`) by ingest outcome: `relayed`, `discarded` when no destination opted in, `shed` when the in-flight relay limit was saturated, or `rejected` for a bad bearer or over-cap body. |
...
| `shunt.router.decisions` | Counter | `model`, `algorithm`, `target`, `source` | Routing decisions for every [`[models.router]`](/reference/configuration/) type, and for the [`[models.subagents]`](/reference/configuration/#modelssubagents-optional) overlay (`algorithm = "subagents"`), by the deciding `[[models]]` entry's id (as matched, so with any `[1m]` hint already stripped), the configured `type`, the model id the decision resolved to, and why it was chosen. `count_tokens` probes are excluded. |
| `shunt.stage_router.decisions` | Counter | `model`, `tier`, `source` | [Stage-router](/guides/stage-router/) tier decisions, by the router's model id (as matched, so with any `[1m]` hint already stripped), the chosen tier (`capable`/`efficient`), and why it was chosen. `count_tokens` probes are excluded. |
| `shunt.router.judge_calls` | Counter | `model`, `algorithm`, `outcome` | Judge calls made by a [`[models.router]`](/reference/configuration/#modelsrouterclassifier-optional) entry that configures one, by the router's model id, the configured `type`, and how the call resolved: `decided`, or one of `timeout`, `oversized`, `upstream_error`, `invalid_reply`, `translation_failed`, `budget_exhausted` (the call was refused because the session spent `max_judge_calls`). On a stage router each of these routes the turn to the picker default; on an `llm_classifier` or `composite` entry, or a classifier-form `[models.subagents]` overlay, the algorithm's own fallback closes it, and a turn answered from a retained assignment with no call records `retained`. On an [`escalation` or `advisor`](/reference/configuration/#buffered-turns-escalation-and-advisor) entry a failure after the completed turn serves that held turn instead, and an advisor review is counted under `algorithm = "advisor"`; the held turn itself is not a judge call. |
...
| `shunt.pool.reprobes` | Counter | `provider` | Reprobes committed at first HTTP dispatch for stale near-quota Codex/ChatGPT accounts; WebSocket-enabled providers count inbound HTTP probes only. |
| `shunt.upstream.status` | Gauge | `provider` | Observed [`[server.status]`](/reference/configuration/#serverstatus-optional) Statuspage severity: `0` none, `1` minor, `2` major, `3` critical. A provider currently `unknown` (no signal) is omitted from this series rather than reported as `0`. |

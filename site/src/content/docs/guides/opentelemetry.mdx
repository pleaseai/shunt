...
| `shunt.stage_router.decisions` | Counter | `model`, `tier`, `source` | [Stage-router](/guides/stage-router/) tier decisions, by the requested model id, the chosen tier (`capable`/`efficient`), and why it was chosen. `count_tokens` probes are excluded. |

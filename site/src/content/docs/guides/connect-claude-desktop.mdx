...
**Discovery filter.** Claude Desktop's auto-discovery shows only *recognizably Claude* ids — the tier-named ones (`claude-sonnet-*`, `claude-opus-*`, `claude-haiku-*`, `claude-fable-*`). shunt's builtin catalog is the superset of what the Anthropic API and the reference Claude apps gateway serve — eleven tier-named ids, so Claude Desktop shows every one. This is the fallback snapshot; when shunt can reach your Anthropic upstream it serves that caller's own live list instead (see [Model Discovery](/guides/model-discovery/)).
...
{ "data": [
  ...
  { "id": "claude-opus-4-1-20250805" }
   ...
   "first_id": "claude-opus-5", "last_id": "claude-opus-4-1-20250805" }
```

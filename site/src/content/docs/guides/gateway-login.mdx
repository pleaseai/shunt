...
The deprecated `jwt_secret_env` (env var name, default `SHUNT_GATEWAY_JWT_SECRET`) and `token_ttl_seconds` (default `3600`) keys remain fully supported when used alone, and `token_ttl_seconds` is still the only way to set a sub-hour lifetime. Combining a deprecated key with its `session.*` replacement fails startup (`jwt_secret_env` with `session.jwt_secret`, or `token_ttl_seconds` with `session.ttl_hours`); shunt only logs a deprecation warning when the config file explicitly sets one of them. See the [configuration reference](/reference/configuration/#servergatewaysession-optional) for the full precedence rules and the secret-rotation procedure.

...
A non-empty telemetry destination list does two things at once. It pushes the
telemetry enable flag plus five `OTEL_*` environment values through managed
settings — including `OTEL_EXPORTER_OTLP_ENDPOINT`, set to your `public_url` —
...
`content-encoding` preserved and the destination's configured headers applied on
top — so protobuf and JSON exporters both work and Claude Code's client-side
attribution attributes survive untouched. The client's `Authorization` header is
never forwarded to a collector.
...
A body over the 32 MiB inbound cap returns `413`.

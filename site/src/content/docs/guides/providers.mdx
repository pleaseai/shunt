...
The Antigravity path runs one CLI subprocess per request in agentic mode. `agy` is a
full agent with its own tools, so it resolves its own tool calls and returns finished
work — file edits included. Three consequences follow:

- **Tools are `agy`'s, not the caller's.** A `tools` array on the Messages request is
  not forwarded, and the response never contains a `tool_use` block. Clients that need
  to drive the tool loop themselves should use the `gemini` provider instead.
- **It runs with `--dangerously-skip-permissions`,** because a non-interactive run has
  no channel to approve a permission prompt. The directory it may touch is resolved
  from `SHUNT_AGY_WORKSPACE`, else a `Working directory:` line in the request's system
  prompt, else the gateway's own working directory. Set `SHUNT_AGY_WORKSPACE` to pin it.
...
turn and clamps an unsupported `effort` to the nearest supported level, preferring the
stronger one — `gemini-3.1-pro` accepts only `low` and `high`, so a route asking for
`medium` runs at `high` rather than failing.

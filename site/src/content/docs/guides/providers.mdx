...
| `antigravity` | `antigravity` | `antigravity_oauth` | `cloudcode-pa.googleapis.com` — reuses `~/.shunt/antigravity-auth.json` (`shunt login antigravity`) |
...
- **Tools are `agy`'s, not the caller's, and a request carrying them is refused.** This
  adapter can never emit a `tool_use` block, so a `tools` array or a `tool_choice` on the
  Messages request is rejected with a `400 invalid_request_error` naming the limitation.
  It used to be dropped silently, which returned a text-only `200` with
  `stop_reason: "end_turn"` — a response an agentic caller cannot act on, and a direct
  violation of `tool_choice: {"type": "any"}` while still reporting success. The one
  exemption is `tool_choice: {"type": "none"}`: the caller has declared it wants no tool
  calls, so a text-only answer is what it asked for. Clients that drive the tool loop
  themselves should use the `antigravity` or `gemini` provider, both of which forward
  tools.
...
and `agy` validates them itself — a failed discovery is retried rather than cached.

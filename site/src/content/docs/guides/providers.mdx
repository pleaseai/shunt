...
#### Pooling several Google accounts

Every `antigravity_cli` entry runs the same local `agy`, which resolves its whole state
tree — credentials included — through `HOME`. Left alone they all share the gateway host's
single ambient `~/.gemini` profile, so every route serves one Google account.

`profile_dir` gives an entry its own `HOME`. Point each at its own directory and each
signs in to its own account, and several can serve concurrently:
...
base_url = "http://localhost"
auth = "none"
# no profile_dir: keeps the ambient ~/.gemini profile

[providers.antigravity-second]
kind = "antigravity_cli"
base_url = "http://localhost"
auth = "none"
profile_dir = "~/.shunt/antigravity/second-account"
```

The directory is created on demand and resolved to an absolute path, and a leading `~` is
expanded. Each one needs its own one-time `agy` sign-in — run the CLI once with that
directory as `HOME`. Unset (the default) changes nothing: the child inherits the gateway's
environment and its ambient profile.

The child environment is also stripped of `GEMINI_API_KEY`, `GOOGLE_API_KEY`,
`GOOGLE_APPLICATION_CREDENTIALS`, `GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_LOCATION`,
`GOOGLE_CLOUD_QUOTA_PROJECT`, `GOOGLE_GENAI_USE_VERTEXAI`, `GCLOUD_PROJECT` and
`CLOUDSDK_CORE_PROJECT`, so the gateway host's own configuration cannot silently decide
which account — and whose billing — serves a request. Stripping applies only to entries
that set `profile_dir`.

The same setting is available on an ordered `[[upstreams]]` entry.
...
Discovery is process-wide, and that matters once `profile_dir` is in play: the matrix is
built by one `agy models` run under the gateway's own environment and shared by every
entry, so it describes the ambient profile's catalog rather than each pooled account's.
Antigravity catalogs do differ per account, so treat the matrix as advisory when pooling
— a model the matrix does not list is passed through to `agy` unvalidated anyway, and
`agy` itself is the authority on what the signed-in account can run.
...
The ChatGPT-account Codex backend **rejects** `gpt-*-codex` slugs — it only accepts the account's live-entitled slugs. The authoritative catalog is openai/codex's [`models.json`](https://github.com/openai/codex/blob/main/codex-rs/models-manager/models.json). Current slugs are `gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna` (latest), `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` (frontier) and `gpt-5.5` / `gpt-5.4`; older accounts may only be entitled to the earlier ones. Use `upstream_model` in a route to map any alias onto an entitled slug.
...
| [OpenCode Zen](/providers/opencode/) | `https://opencode.ai/zen` | `claude-fable-5-1`, `gpt-6-astra` — curated cross-vendor catalog (reads `x-api-key`) |
...
| :-- | :-- | :-- |
| `shunt-codex` | `gpt-6-sol`, `gpt-6-luna`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` | `codex` (ChatGPT subscription) |

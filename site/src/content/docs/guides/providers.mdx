...
credentials, and egress underneath a green startup would be worse than failing. Run `shunt login antigravity`, or point the route at
`antigravity-cli`. Startup also logs a warning whenever a route still resolves to the deprecated `antigravity_cli` kind, so operators

...
If shunt runs [`[server.gateway]`](/guides/gateway-login/), each user can carry their own gateway session instead of a shared token. Sign in once, then use the second helper:
...
Then start `claude`, run `/status`, and check the **Anthropic base URL** line shows your gateway. See also [Effort & Context](/guides/effort-and-context/) for reasoning-effort and context-window tuning, and [Client Environment Variables](/reference/client-environment/) for the environment variables that change behind a gateway.
...
Configure `[server.weekly_fallback]` to retain the selected provider until its enabled account pool exhausts shared weekly quota. The explicit pairs support both directions on Messages. Fable can use Opus after an eligible failure. The policy is off by default. See the [configuration reference](/reference/configuration/#serverweekly_fallback-optional) for all pairs and conditions. The inbound Codex endpoint and `count_tokens` retain their existing behavior.

...
field `service_tier: "priority"`, branded "Fast" (1.5x speed, increased usage). shunt exposes the
same knob as a config key, at both provider and route level, exactly parallel to `effort`:
...
(a client-only sentinel meaning "unset" — never sent on the wire). It's off by default and sent
only when explicitly configured; a route-level value wins over the provider-level default, same as
`effort`. Full key reference: [Configuration](/reference/configuration/#providersname).

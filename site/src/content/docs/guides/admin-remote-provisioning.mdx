...
Managed account provisioning is a separate advanced action. Expanding **Manage pool accounts** lets you create shunt-owned Claude or Codex credentials for load-balancing. These managed copies live in `~/.shunt/accounts/` and shunt owns their refresh/writeback; they are not required merely to view local usage.
...
The primary **Accounts and usage** table is automatic and keyed by recognizable masked identity rather than a store filename. Each row uses its provider's native signal:

...
- the upstream request is aborted at the match rather than left running. Tokens the upstream
  generated between the match and the abort are still billed by the upstream — negligible for the
  short completions stop sequences are typically used for.

One consequence to be aware of: because the turn ends before the upstream's `response.completed`
event, its usage report never arrives. The `message_delta.usage` of such a turn carries shunt's
local input estimate and `output_tokens: 0`.

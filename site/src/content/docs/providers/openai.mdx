...
the Responses path onto the native client-executed `tool_search` protocol by default, on GPT-5.4+
models; set `tool_search = false` on the provider to fall back to the text shim instead. See [Tool
search](/guides/codex/#tool-search).

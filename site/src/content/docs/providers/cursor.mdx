...
Treat `tool_use.id` as opaque. When Cursor identifies the call on the wire, shunt passes that
identity straight through, so the id carries Cursor's own `tool_<uuid>` shape rather than the
`toolu_<uuid>` an Anthropic upstream would mint; when it does not, shunt mints a `toolu_<uuid>`
itself. Either way the id only has to round-trip: send it back unchanged as the `tool_result`
`tool_use_id`.
...
A turn also ends only on an authoritative terminal from Cursor. If the upstream stream is cut
before that terminal arrives — a dropped connection, a truncated body — shunt fails the turn
instead of handing back the partial answer as a finished one:

```json
{"type":"error","error":{"type":"api_error","message":"cursor: upstream EOF without an authoritative terminal"}}
```

The exception is a turn that has already produced output and then goes quiet: Cursor holds the
response open waiting for a tool result the stateless bridge never sends, so an idle stream there
is the normal end of a turn, not a cut one.

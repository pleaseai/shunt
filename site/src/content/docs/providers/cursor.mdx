...
```json
{"type":"error","error":{"type":"api_error","message":"cursor used one of its own built-in tools instead of a caller-supplied tool, which shunt cannot bridge; retry, or send a request with no tools"}}
```

The choice is not deterministic — retrying the same request usually lands on the bridged path. A
request that advertises no tools is unaffected.

...
provider**, never a route to a different one. The classifier request carries `stop_sequences`, which
shunt's Responses translation drops: pointed at a Codex upstream through the Sonnet alias, the
classifier retried its first stage and every permission check took 5–7 s longer. An unset key (the
default) leaves the classifier on whatever model the client asked for.

...
| `GET` | `/admin/api/routes` | ダッシュボードが描画するルーティング表 — 上の `GET /routes` が返すものと同じボディで、同じスナップショットから構築されるため、運用者が見る表とクライアントが実際に解決する表が食い違うことはありません。リダイレクトではなく個別に登録しているのは、[`[server.admin]`](/reference/configuration/#serveradminオプション) が管理面を独自のリスナーにバインドでき、そのリスナーには `/routes` が登録されないためです。認証のない公開エンドポイントとは異なりこちらは管理者資格情報を要求するので、`/routes` を開放していても当該資格情報が守る範囲は広がりません |

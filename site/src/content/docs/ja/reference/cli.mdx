...
shunt はデプロイの `/.well-known/oauth-authorization-server` ドキュメントを読み、そこで告知されたエンドポイントを使います。確認用 URL と短いユーザーコードを表示してブラウザを開き、ゲートウェイの `/device` ページで承認されるまでポーリングします。discovery が `404` を返す場合、そのデプロイに `[server.gateway]` セクションが無いことがほとんどです。discovery と device code 要求にはそれぞれ上限があり、接続だけ受け付けて応答しないデプロイはターミナルを無期限に待たせる代わりにタイムアウトとして報告されます。その後の承認ポーリングは待つことが役目なので、同じようには制限しません。ループバック以外の平文 `http://` URL も受け付けますが警告が出ます — device code と refresh token が暗号化されずに流れるためで、しかもこのログイン時だけではありません。以降のトークン更新もすべて同じ経路を通り、`shunt gateway token` は実際に更新を行うたびに同じ警告を再度表示します（ネットワーク呼び出しの無いキャッシュトークン返却時には表示しません）。

デプロイが返してくる内容には 2 つの下限が適用されます。shunt が device code と refresh token を POST するのは、discovery ドキュメントが `https` で、またはループバックアドレスに対して `http` で告知したエンドポイントだけです。それ以外のエンドポイントは資格情報を受け取る代わりに、名前を挙げて discovery が失敗します。また確認用 URL を自動的に開くのは `http` か `https` のときだけです — その文字列を選ぶのは shunt ではなくゲートウェイなので、他のスキームなら開かなかった理由とともに表示し、たどるかどうかは利用者が判断します。
...
保存済みトークンは有効期限の 5 分前まではそのまま使われ、それ以降にリフレッシュされます。ゲートウェイの refresh token はローテーションする使い捨てなので、リフレッシュは排他ファイルロックの下で実行され、ロック取得後にセッションを読み直します: 複数の Claude Code セッションが同時にヘルパーを呼んでもリフレッシュは 1 回だけで、ローテーションファミリー全体を失効させる再送は起きません。
...
5 分のバッファに入ったことは、リフレッシュが**必要な時期**であることを意味し、**必須**という意味ではありません。キャッシュされたトークンがまだ実際に有効なうちにリフレッシュが失敗した場合、短いネットワーク障害を認証エラーに変える代わりに、stderr に警告を出したうえでそのトークンを返します。実際に期限が切れた後はコマンドが失敗します。Claude Code のヘルパー検証器が拒否するトークン — 空白を含まない印字可能 ASCII 1〜16384 文字以外 — は、何の説明もなく認証を失敗させるために出力される代わりに、ゲートウェイを名指しする診断とともにここで拒否されます。`shunt gateway logout` はセッションと一緒にロックファイルも削除します。
...
完全な upstream 設定は [Kimi](/ja/providers/kimi/#kimi-code-oauth-subscription) を参照してください。

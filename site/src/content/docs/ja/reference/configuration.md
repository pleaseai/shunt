...
`PYO3_PYTHON` をそのインタープリター（3.7 以上、共有 libpython つき）に設定してください。
...
`max_length` や `batch_size` も起動エラーです。この検査はフィーチャーの有無にかかわらず
どのビルドでも行われ、重複ターゲットは他のターゲット比較と同じく末尾の `[1m]`/`[1M]`

...
`availableModels` がリクエストを通した後だけで、その判定はルーターが選ぶ一つのターゲットではなく、
エントリが名指しするすべてのターゲットに対して行われます。したがって未認証の呼び出し元がこの
id を指定しても推論は走らず、送られてきたセッション id にアフィニティが記録されることもありません。

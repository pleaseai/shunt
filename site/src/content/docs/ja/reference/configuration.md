...
| キー | デフォルト | 意味 |
| :-- | :-- | :-- |
| `capable_target` | ✅ 必須 | 難しい推論・調査・エラー回復を担うモデル id |
| `efficient_target` | ✅ 必須 | 計画が固まった後の定型作業を担うモデル id |
| `picker` | `efficient_first` | シグナルが決め手に欠けるときに使うティア。`efficient_first` または `capable_first` |
| `confidence_threshold` | `0.5` | シグナルに従って動くために必要な最小のスコアラー確信度。範囲は `(0.0, 1.0]` |
| `recent_turn_window` | `3` | スコアラーに渡す tool result のアシスタントターン数。最小 `1` |
| `min_dwell_turns` | `3` | 下降が発火できるようになるまでティアを保持するターン数 |
| `deescalate_threshold` | `0.75` | ティアを*下げる*のに必要な確信度。`confidence_threshold` より意図的に厳しくしてあります |
| `session_ttl_seconds` | `3600` | 静かなセッションの固定ティアが生き残る時間 |

ターゲット自身がルーターである場合、空のターゲット、`(0.0, 1.0]` を外れたしきい値、`0` の
`recent_turn_window`、`[1m]` を含むルーターの **id**、同じエントリに
`[models.upstream_model]` も宣言した場合は起動時エラーです。明示的なルートに一致しない
ターゲットは警告のみです。一致しない他の id と同様に `server.default_provider` で解決
されるためです。

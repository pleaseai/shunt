...
`count_tokens` プローブにはノートが付きません。空のノートは起動エラーです。
...
| `weights` | 均等 | ターゲットごとに 0 以上の重みを 1 つ。`0` はそのターゲットを無効にします |

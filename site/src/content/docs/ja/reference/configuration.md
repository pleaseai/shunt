...
`passthrough` アップストリームでは、クライアント自身の `authorization` / `x-api-key` は宛先オリジンがプライマリアップストリームと一致する間のみ転送されます。この資格情報はオリジン固有であるため、**異なる**オリジンへの `passthrough` フェイルオーバー試行ではこれを削除してフェイルクローズし、ホスト固有のトークンを別のオリジンへ再送しません。同一オリジンのフォールバック（例：1 つのホスト上の 2 つの passthrough エントリ）は引き続き資格情報を保持します。`api_key`／OAuth アップストリームは位置に関係なく自前のサーバーサイド資格情報を注入します。

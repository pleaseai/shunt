...
チェーン解決後、リクエストが当該アダプターでは扱えない既知の機能（tools、画像、structured output、明示的な reasoning effort、`[1m]`/`[1M]` コンテキストヒント）を必要とする場合、後続要素は除外されます。プライマリは常に残します。`[1m]`/`[1M]` ヒントは、検証済みの 1M 経路を広告するアダプターがないため、現時点ではすべてのフォールバックを除外します。除外は `capability_excluded` として記録され、最良失敗としては記憶しません。

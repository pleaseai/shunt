...
`passthrough` アップストリームでは、クライアント自身の `authorization` / `x-api-key` は**最初の**試行にのみ転送されます。この資格情報はプライマリアップストリームのホスト固有であるため、別のホストへの後続の `passthrough` 試行ではこれを削除してフェイルクローズし、ホスト固有のトークンを別のオリジンへ再送しません。`api_key`／OAuth アップストリームは位置に関係なく自前のサーバーサイド資格情報を注入します。

...
| マッピングされたモデルで `400 Invalid schema for function '…': '…' is not a 'regex'` | OpenAI バックエンドはツールスキーマの各 `pattern` を Python の `re` でコンパイルするため、JavaScript 専用の正規表現(`\p{Cc}`、`(?<name>…)`、`\u{…}`)を拒否します。Claude Code の `Artifact` ツールがそのようなパターンを含みます。shunt は転送するスキーマからこれらのパターンを取り除く(制約は `description` に残る)ので、まだ表示される場合は更新してください。 |

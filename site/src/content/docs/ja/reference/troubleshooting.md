...
| マッピングされたモデルで `400 Invalid schema for function '…': '…' is not a 'regex'` | OpenAI バックエンドはツールスキーマの各 `pattern` を Python の `re` でコンパイルするため、JavaScript 専用の正規表現（`\p{Cc}`、`(?<name>…)`、`\u{…}`）を拒否します。Claude Code の `Artifact` ツールがそのようなパターンを含みます。shunt は転送するスキーマからこれらのパターンを取り除きます。strict モード外では `pattern` は助言的なのでヒントが失われるだけですが、`description` が常にその制約を書き直しているわけではありません。まだ表示される場合は更新してください。 |
| `400 Invalid JSON schema: regex lookaround is not supported` | ChatGPT のスキーマコンパイラは Python が受理する先読み・後読みも拒否します。このローカル互換性パッチは Neon `create_auth_user` のメールパターンを含め、該当パターンを送信するツールスキーマから除去します。ツール実行とサーバー側の入力検証は変更しません。 |
| マッピングされたモデルでツール呼び出しがすべてのオプションパラメータ埋めで届く | バックエンドは明示的な `strict` のない function ツールを strict モードへ正規化し、そのモードでは閉じたパラメータオブジェクト(`additionalProperties:false`)がすべてのプロパティを必須のように扱うため、呼び出し元が設定していない値までモデルが埋めます。shunt は転送する function ツールと `tool_search_output` 内で再生されるスペックに `strict:false` を固定してオプションをオプションのまま保ちます; xAI/Grok ルートでは未検証のためフィールドを送らず、ネイティブ `tool_search` はスキーマにオプションプロパティがないため省略します。まだ表示される場合は更新してください。 |

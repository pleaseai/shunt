...
| `shunt.router.decisions` | カウンター | `model`, `algorithm`, `target`, `source` | すべての [`[models.router]`](/ja/reference/configuration/) `type` のルーティング決定。ルーターの model id（マッチした形なので `[1m]` ヒントは除去済み）、設定された `type`、決定が解決したモデル id、選んだ理由ごとに数える。`count_tokens` のプローブは除外。 |

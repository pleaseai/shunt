...
| `shunt.stage_router.decisions` | カウンター | `model`, `tier`, `source` | [ステージルーター](/ja/guides/stage-router/)のティア決定。リクエストされた model id、選ばれたティア（`capable`/`efficient`）、選んだ理由ごとに数える。`count_tokens` のプローブは除外。 |

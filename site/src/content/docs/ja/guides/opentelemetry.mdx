...
このページは、外へ出ていく shunt **自身の** テレメトリーについてのものです。ゲートウェイの独立した[テレメトリー受信](/ja/guides/gateway-login/#テレメトリーの受信)は逆方向 — 管理された Claude Code **クライアント** が shunt へ送る OTLP ペイロード — を扱い、`[server.gateway.telemetry]` の宛先へ verbatim に中継します。両者は独立して設定され、異なるコレクターを指すこともできます。
...
| 系列 | 種類 | 属性 | 意味 |
| :-- | :-- | :-- | :-- |
| `shunt.requests` | カウンター | `provider`, `model`, `http.response.status_code`, `caller` | プロキシされた推論リクエスト。`caller` はインバウンドのトラフィックなら `client`、[`[models.router]`](/ja/reference/configuration/) エントリが呼び出し元に代わって行ったジャッジ呼び出しなら `router`。 |
| `shunt.latency` | ヒストグラム(ms) | `provider`, `model`, `http.response.status_code`, `caller` | ストリームではヘッダーまで、それ以外では応答全体のレイテンシ。コミット型ストリーミング応答はヘッダーがアップストリーム応答より先にコミットされるため、ディスパッチから試行分類までを測定します。 |
| `shunt.ttft` | ヒストグラム(ms) | `provider`, `model` | リクエスト開始から最初の完全な非キープアライブ SSE フレームまでの時間。 |
...
| `shunt.gateway_telemetry_ingest` | カウンター | `signal`, `outcome` | インバウンドのゲートウェイ OTLP ペイロード(`metrics`/`logs`/`traces`)を受信結果別に集計: `relayed`、どの宛先も opt-in していなければ `discarded`、飛行中の中継上限で飽和していれば `shed`、不正なベアラーや上限超過ボディは `rejected`。 |
...
| `shunt.router.decisions` | カウンター | `model`, `algorithm`, `target`, `source` | すべての [`[models.router]`](/ja/reference/configuration/) `type` のルーティング決定と、[`[models.subagents]`](/ja/reference/configuration/#modelssubagentsオプション) オーバーレイの決定（`algorithm = "subagents"`）。決定を下した `[[models]]` エントリの id（マッチした形なので `[1m]` ヒントは除去済み）、設定された `type`、決定が解決したモデル id、選んだ理由ごとに数える。`count_tokens` のプローブは除外。 |
| `shunt.stage_router.decisions` | カウンター | `model`, `tier`, `source` | [ステージルーター](/ja/guides/stage-router/)のティア決定。ルーターの model id（マッチした形なので `[1m]` ヒントは除去済み）、選ばれたティア（`capable`/`efficient`）、選んだ理由ごとに数える。`count_tokens` のプローブは除外。 |
| `shunt.router.judge_calls` | カウンター | `model`, `algorithm`, `outcome` | ジャッジを設定した [`[models.router]`](/ja/reference/configuration/#modelsrouterclassifierオプション) エントリが行ったジャッジ呼び出し。ルーターの model id、設定された `type`、呼び出しがどう決着したか（`decided`、または `timeout`、`oversized`、`upstream_error`、`invalid_reply`、`translation_failed`、`budget_exhausted` のいずれか）ごとに数える。`budget_exhausted` はセッションが `max_judge_calls` を使い切ったため呼び出しが拒否されたことを表す。ステージルーターではこれらの結果はいずれもそのターンを picker の既定値にルーティングし、`llm_classifier` や `composite` のエントリ、classifier 形式の `[models.subagents]` オーバーレイではアルゴリズム自身のフォールバックがターンを閉じる。呼び出しなしで保持された割り当てから応答したターンは `retained` として記録される。[`escalation` や `advisor`](/ja/reference/configuration/#留め置くターン-escalation-と-advisor) のエントリでは、完成したターンのあとの失敗は留め置いたそのターンを代わりに返し、advisor のレビューは `algorithm = "advisor"` として数える。留め置いたターン自体はジャッジ呼び出しではない。 |
| `shunt.stage_router.flips` | カウンター | `model`, `from`, `to` | セッションを固定済みのティアから動かした決定。昇格は容易に、降格は難しくという設計なので、2 つの方向を分けて数える。 |
| `shunt.requests_shed` | カウンター | — | `max_concurrent_requests` の上限で拒否されたインバウンドリクエスト。拒否自体は `debug!` でのみログ出力されるため、ゲートウェイの飽和を可視化するのはこのカウンターです。 |
...
| `shunt.pool.reprobes` | カウンター | `provider` | 陳腐化した近接クォータの Codex/ChatGPT アカウントで、最初の HTTP 送信時にコミットされた再プローブ。WebSocket 有効プロバイダーは inbound HTTP のみを数える。 |
| `shunt.upstream.status` | ゲージ | `provider` | 観測された [`[server.status]`](/reference/configuration/#serverstatus-optional) Statuspage severity: `0` none、`1` minor、`2` major、`3` critical。現在 `unknown`（信号なし）の provider は `0` として報告されるのではなく、この系列から完全に除外される。 |

...
`grok-4.5` は `reasoning.effort` を受け入れます（ライブ検証済み）。それで `400` になるスラッグでは `effort` を未設定のままにしてください。完全な優先順位とエフォートの表: [Effort & Context](/ja/guides/effort-and-context/#reasoning-エフォート)。

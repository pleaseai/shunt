...
shunt はリクエスト/レスポンス本文・ヘッダー・認証情報を一切エクスポートしません。

- **メトリクスとトレース** は低カーディナリティで本文を含みません。リクエストスパンのクライアント **セッション id** は、`include_session_id = true`(デフォルトはオフ)のときのみ、しかもトレースエクスポートが有効な間だけ付与されます。
...
エクスポートされるリソースは `service.*` と `telemetry.sdk.*` のみを広告します — host や process detector は動かないため、マシンのホスト名は付与されません — これに標準の `OTEL_RESOURCE_ATTRIBUTES` で設定した値が加わります。

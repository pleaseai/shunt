...
認証情報は `SHUNT_CLIENT_TOKENS` と同じカンマ区切りの `name:token` 形式を使いますが、別個のセキュリティ境界です。`[server.auth]` のクライアントトークンを管理トークンとして再利用しないでください。`[server.admin]` が存在するのに 3 つの認証情報ソース（トークン環境変数または `tokens_file`、`[[server.admin.write_keys]]`、`[[server.admin.read_keys]]`）がすべて空または不正な場合、起動はフェイルクローズします。`name:token` ペアは **write** ティアで、`read_keys` のエントリーは管理サーフェスのすべての GET を通過しますが、ログインフォームを含むすべての変更操作では拒否されます。

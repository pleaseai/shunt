...
認証情報は `SHUNT_CLIENT_TOKENS` と同じカンマ区切りの `name:token` 形式を使いますが、別個のセキュリティ境界です。`[server.auth]` のクライアントトークンを管理トークンとして再利用しないでください。`[server.admin]` が存在するのに 3 つの認証情報ソース（トークン環境変数または `tokens_file`、`[[server.admin.write_keys]]`、`[[server.admin.read_keys]]`）がすべて空または不正な場合、起動はフェイルクローズします。`name:token` ペアは **write** ティアで、`read_keys` のエントリーは管理サーフェスのすべての GET を通過しますが、すべての変更操作では拒否されます。ダッシュボードにサインインすることはでき、そのセッションは読み取り専用です。
...
:::note
`/admin` が返すダッシュボードは、`--features ui` でビルドしたバイナリだけが埋め込むバンドルから配信されます。ビルド済みリリースバイナリと Homebrew formula には含まれますが、素の `cargo build` や `cargo install` には含まれず、そのパスはこの機能名を含む本文とともに `404` を返します。ソースからのビルドは [インストール](/ja/getting-started/installation/) を参照してください。このページのそれ以外 — JSON API と設定 — はどちらでも同じです。
:::
...
開始されたフローは `pending_ttl_secs`（デフォルト 10 分）の間有効で、オペレーターが認可ページを開いて結果を貼り付ける時間を確保できます。サーバーは選択された mode を pending attempt とともに記録するため、completion リクエストで token の種類を切り替えることはできません。**Start** を選んだ時点からフローが完了するまで、ダッシュボードはログイン方法のラジオボタンを無効化し、その理由も表示します。pending attempt の mode はリクエストを送る時点で固定されるためです。**Start** をもう一度選んでも解除されません。フローを完了するか、ページを再読み込みすると再び選べるようになります。Full OAuth は access token と refresh token を保存し、credential kind は `imported` と表示されます。setup-token mode は kind が `setup_token` の静的 credential を保存します。完了レスポンスは、アカウントが保存されたか、そして現在の provider 設定でそれが有効（live）になるかを報告します。
...
### 死んだクレデンシャルと Refresh ボタン

オペレーターの介入なしにはどのリトライでも回復しないアカウントは、`cooling` ではなく **Needs re-login** と表示されます。主テーブルと高度なプールテーブルの両方でです。この区別が重要なのは、クールダウンは 5 分で自然に失効してアカウントが再び試されるため、恒久的に死んだクレデンシャルがクォータ一時停止と**永久に**同じに見えてしまうからです。この印は、プロバイダーが refresh グラントを終端的に拒否したとき、リフレッシュ自体が不可能なクレデンシャル（`token_env` トークンや期限切れの setup token）が拒否されたとき、そしてリフレッシュ直後の bearer で再試行したリクエストが**なお**拒否されたときに立ちます。クールダウンとは独立に報告され、メモリ上のみのため、再起動するとそのアカウントが次に失敗するまでクリアされます。この印は、つまずいたプロバイダーエントリではなく**クレデンシャル**に従います。1 つの保存済みアカウントを複数のプロバイダーテーブルで名前によって有効化している場合、その行はすべて一緒に切り替わります。自分の `credentials` ファイルや `token_env` を指すアカウントは、名前が同じでも別のクレデンシャルなので、そこでの失敗は他の何にも印を付けません。

回復手段はアカウントを削除して追加し直すことです。`imported` アカウントの行には **Refresh** ボタンもあります。そのアカウントの refresh グラントを即座に実行してログインがまだ生きているかを報告するので、トラフィックを待たずに再ログインの成否を確認するのに便利です。プロバイダーのトークンエンドポイントを叩くためレート制限があり、共有クレデンシャルストアを必ず経由するのでプロキシ自身のリフレッシュと競合せず、通常の利用と**同じように保存された refresh token をローテーションします**。新しい有効期限のみを返し、トークン本体は一切返しません。`setup_token` アカウントは refresh グラントを持たないため、プロバイダーに接続せず `400` で応答します。

プローブの成功が印を消すのは、**グラントの失敗が印を立てた場合のみ**です。プロバイダーが既に発行した bearer を拒否したことでアカウントが死んだと判定された場合、グラントが動くという事実は推論を提供できることを何ら証明しないため、印は残り、レスポンスもそう伝えます。
...
ダッシュボードの Claude テーブルは、この有効期限を認証情報の種類から導出した **Status** として表示します。生の値は約 8 時間の *access* token の期限であり、種類によって意味が正反対になるためです。`imported` アカウントは refresh token を持ち shunt が必要に応じて更新するので **Auto-refreshes** と表示され（生のタイムスタンプはホバーで確認）、過去の日付でも正常です。`setup_token` アカウントは更新自体ができないため、1 年の有効期限は実際に対処が必要です。更新バッファより余裕があれば **Valid until** と日付、バッファ内に入るか日付を過ぎていれば danger スタイルの **Expired** と再ログインの案内が表示されます。バッファはルーティングが適用するものと同じ 5 分（`EXPIRY_BUFFER`）なので、ルーティングがすでに refresh token なしの経路で失敗させる区間を、ダッシュボードが利用可能と表示することはありません。各行には **Re-login** ボタンもあり、上の追加フォームにそのアカウントの名前とログイン方法を入力します — 既存の名前で通常のフローを完了すると、そのアカウントはその場で上書きされます。Codex テーブルにも同じ **Status** 列がありますが、こちらは無条件です。その store には更新できない種類が存在せず（書き込み経路が 2 つとも refresh token を必須とします）、shunt がすべての Codex アカウントを更新するため、各行は **Auto-refreshes** と表示され、生のタイムスタンプは同様にホバーで確認できます。Codex の行にも同じ **Re-login** ボタンがありますが、ログイン方法の事前選択はありません — その store への入口は ChatGPT OAuth だけだからです。上部の使用量テーブルの読み取り専用の行は影響を受けません。そちらの認証情報を更新するのは shunt ではなく、所有している provider クライアントです。

アカウントの metadata、プールの健全性、プロビジョニング、またはアカウント削除に API/curl でアクセスするには、設定したヘッダー（デフォルト `x-shunt-admin-token`）または `x-api-key` で管理認証情報を送り（どちらのスロットも管理ルートでのみ受け付けられ、推論ルートでは受け付けられません）、[HTTP エンドポイント](/ja/reference/endpoints/)に記載の JSON route を使ってください。ヘッダー認証されたリクエストはブラウザーセッションを使わず、CSRF チェックの対象外です。プロビジョニングの開始時には `{ "name": "backup", "mode": "oauth" }` または `mode: "setup_token"` を送ります。`mode` を省略した場合は、API の後方互換性のため `setup_token` がデフォルトです。

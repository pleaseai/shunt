...
**Discovery フィルター。** Claude Desktop の自動 discovery に表示されるのは、*Claude と認識できる* id、つまり tier 名の id（`claude-sonnet-*`、`claude-opus-*`、`claude-haiku-*`、`claude-fable-*`）のみです。shunt の組み込みカタログは Anthropic API とリファレンス Claude apps gateway が提供する一覧の上位集合です。13 個すべてが tier 名の id なので、Claude Desktop にはそのすべてが表示されます。これはフォールバック用のスナップショットで、shunt が Anthropic アップストリームに到達できる場合は、その呼び出し元自身の実際の一覧を返します（[モデルディスカバリー](/ja/guides/model-discovery/) を参照）。

...
- デフォルトの切り替えしきい値は `0.98` です。5 時間の status が `rejected`、適用される週次の status が `rejected`、共有 5 時間の使用率がそのしきい値に達している、または適用される週次使用率がそのしきい値に達しているとき、アカウントはクォータに近い状態です。Fable モデルでは、適用される週次の status は `7d_oi` の使用率があれば `7d_oi`、なければ共有 `7d` です。集約 status は、ウィンドウ別の status が 1 つも記録されていない場合にのみ使われる互換用のフォールバックです。しきい値は、アカウント単位（上記の `threshold*` フィールド）またはプール全体（[選択のチューニング](#選択のチューニングserverpool)を参照）で下げられます。
...
| :-- | :-- |
| 2xx | 中継し、健全としてマークします。 |
| Fable リクエストで `anthropic-ratelimit-unified-7d_oi-status: rejected` だけの 429 | Fable のクォータ枯渇：数値の `retry-after` で Fable リクエストのみに効くクールダウン（デフォルト 60 秒、1〜3600 秒にクランプ）を適用し、その後ローテーションします。Fable 以外のリクエストは引き続きそのアカウントを使えます。 |

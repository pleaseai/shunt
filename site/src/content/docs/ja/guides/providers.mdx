...
3 つめの設定である `profile_dir` は、エントリーごとに専用の `HOME` を与えます。`agy` は認証情報を含む状態ツリー全体を
`HOME` 経由で解決するため、未設定のままではすべてのエントリーがゲートウェイホストの単一の ambient な `~/.gemini`
プロファイルを共有し、結局ひとつの Google アカウントしか使えません。エントリーごとにディレクトリを指定すれば、それぞれが
自分のアカウントにサインインし、複数を同時にプールできます。ディレクトリは必要に応じて作成され絶対パスに正規化されます。
先頭の `~` は展開されます。各ディレクトリには一度ずつ `agy` のサインインが必要です。この設定を使うエントリーに限り、子
プロセスの `GOOGLE_*` / `GEMINI_*` 環境変数が取り除かれるため、ホスト側の設定がどのアカウント — そして誰の請求 — が
リクエストを処理するかを黙って変えてしまうことはありません。未設定（既定）なら従来どおりの動作です。順序付き
`[[upstreams]]` エントリーでも同じ設定を利用できます。
...
| [OpenCode Zen](/ja/providers/opencode/) | `https://opencode.ai/zen` | `claude-fable-5-1`, `gpt-6-astra` — キュレーションされたクロスベンダーカタログ(`x-api-key` を読む) |
...
| :-- | :-- | :-- |
| `shunt-codex` | `gpt-6-sol`、`gpt-6-luna`、`gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna` | `codex`（ChatGPT サブスクリプション） |

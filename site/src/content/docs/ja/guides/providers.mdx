...
ツール呼び出しを自分で解決するため、リクエストの `tools` 配列は転送されず、レスポンスに `tool_use` ブロックが含まれることも
ありません。ツールループを自分で回す必要があるクライアントは、`gemini` プロバイダーを使用してください。
...
これらを一切必要としない、上記のネイティブな `antigravity` HTTP プロバイダーの使用を推奨します。詳細は
[非推奨の antigravity-cli 転送](/guides/providers/#the-deprecated-antigravity-cli-transport) を参照してください。
...
| [Kimi Code（サブスクリプション、OAuth）](/ja/providers/kimi/#kimi-code-oauth-subscription) | `https://api.kimi.com/coding` | サブスクリプションが公開しているモデル ID を使用 |
...
`kimi_oauth` は `claude_oauth` / `chatgpt_oauth` と同様にプール対応です — `account` の代わりに `accounts = [...]` を使うと、保存済みの複数の Kimi アカウントへ負荷を分散できます。詳しい手順は [Kimi → Kimi Code (OAuth subscription)](/ja/providers/kimi/#kimi-code-oauth-subscription) を参照してください。

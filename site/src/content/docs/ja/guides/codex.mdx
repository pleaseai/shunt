...
有効にすると、shunt はデフォルトでツール検索を OpenAI Responses API 自身の **ネイティブなクライアント実行
`tool_search`** プロトコルへマッピングします。Claude Code の `ToolSearch` ツールは `tool_search`
（`execution: "client"`）ツールになり、その `tool_use` は `tool_search_call` になり、`tool_reference`
の結果はロードされたツールの完全なスキーマを構造化 JSON として運ぶ `tool_search_output` アイテムとなって
...
- デフォルト動作のために `shunt.toml` を変更する必要はありません — 純粋に `ENABLE_TOOL_SEARCH` レベルの
  Claude Code 環境変数です。
- `codex`(ChatGPT)および `openai`(標準 Responses)プロバイダーに適用されます。
...
`tool_search` は `[providers.<name>]` の下でデフォルト `true` です。ネイティブな形状は ChatGPT/Codex
バックエンド(gpt-5.6、2026-07-13、[pleaseai/shunt#86](https://github.com/pleaseai/shunt/pull/86))
に対してライブプローブで検証済みであり、`tool_search` は公開 OpenAI
Responses API の gpt-5.4+ モデル向けドキュメントにも記載されています。ネイティブパスに対応しているプロバ
イダーでテキストシムへ戻したい場合は `tool_search = false` を設定します:
...
| ツール検索がコンテキストを削減しない(まだシムのキャッシュ無効化コストを払っている) | フレーバー / モデルのゲートを確認する — ネイティブな `tool_search` には標準 OpenAI/ChatGPT-Codex 系のプロバイダーと gpt-5.4 以降のモデルが必要で、非対応の組み合わせは静かにシムへフォールバックします。両方のゲートを満たしているのに削減されない場合は `tool_search = false` を設定していないか確認する。上記の [ツール検索 → ネイティブプロトコル](#ネイティブプロトコル) を参照。 |

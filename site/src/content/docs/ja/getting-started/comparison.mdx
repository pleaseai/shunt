...
shunt は**仕様準拠の Claude Code LLM ゲートウェイ**です。Claude Code の公式 `ANTHROPIC_BASE_URL` ゲートウェイ契約（`/v1/messages`、`/v1/models` discovery、アトリビューション/ヘッダーのパススルー）を実装し、**`model` id 単位の選択的**な迂回を行います — メインセッションは Claude に保ったまま、指名したモデルだけを別のプロバイダー（ChatGPT/Codex、OpenAI、xAI）へ振り向けます。マッピングされたモデルについては Anthropic Messages ⇄ OpenAI Responses API を変換し、それ以外はすべて Anthropic へ無変更でパススルーします。ルーティングはリクエストの `model` id によるもので、プロンプト形状のフィンガープリンティングはありません（`README.md:104-131`）。唯一の例外はオプトインであり、プロンプトのテキストは一切読みません。`type = "stage_router"` を宣言した [`[models.router]`](/ja/guides/stage-router/) エントリは構造化された tool-result メタデータから 2 つのティアのどちらかを選びます。これは次の Claude Code リリースが書き換えるプロンプトではなく、安定したワイヤ契約です。同じテーブルの他の type — `auto`、`random`、`noop` — はリクエストボディを一切読みません。
...
- **A. tool-search のコンテキスト節約（追跡中: [#43]）。** **[#82]/[#286] でおおむね対処済み、[#289] で範囲を調整**: プロバイダー別 `tool_search` フラグは、デフォルト（「auto」）では、すでにサポートが確認されているホスト — ChatGPT/Codex バックエンドと `api.openai.com` — が gpt-5.4+ モデルへルーティングする場合に限り、tool search をテキストシムの代わりに Responses API のネイティブでクライアント実行の `tool_search` プロトコルへマッピングします（上の脚注 8 参照）。それ以外の OpenAI 互換エンドポイントは明示的な `tool_search = true` のオプトインが必要です。残るギャップは対象外のフレーバー/モデル/ホストです: xAI/Grok のルート、gpt-5.2 以下のモデル、そしてまだオプトインしていないカスタムの OpenAI 互換エンドポイントは、いずれも依然としてシムへフォールバックします。シムは未ロードの遅延ツールを `tools` 配列から除外するため（`src/model/responses_request.rs:819-830`）、二度と明らかにされないツールについては実際にコンテキストを取り戻します — しかし Claude Code がツールを一つ明らかにすると、シムはその完全なスキーマを `tool_reference` テキストとしてレンダリングし（`:637-661`）、*さらに* そのツールを `tools` に再追加するため、明らかにするたびにスキーマが再送信され、それ以降キャッシュ済みプロンプトプレフィックスが無効化されます。Responses API はモデルが `tools` に見えているツールしか呼び出せないため、この二重送信はネイティブプロトコルなしには避けられません — 残存ギャップは、xAI/Grok と旧世代の OpenAI モデルがアップストリームで独自の `tool_search` 相当のサポートを備えるか、より多くのカスタムエンドポイントが検証されてオプトインすることで初めて解消されます。

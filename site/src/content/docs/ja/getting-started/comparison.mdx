...
| tool-search / `defer_loading` / `tool_reference` の処理 | ◐（標準 OpenAI/Codex かつ gpt-5.4+ ではネイティブがデフォルト⁸。それ以外はシム） | ○⁵ | ◐（上流）/ ●（フォーク） | ○ | ○ |
...
⁸ **[#82]** はプロバイダー別 `tool_search` フラグ（`src/config.rs:1136-1149,2792-2805`）を追加し、スキーマをテキストへ折り込む代わりに、Claude Code の tool search を OpenAI Responses API 自身のネイティブでクライアント実行の `tool_search` プロトコルへマッピングします — `ToolSearch` → `tool_search`、その `tool_use` → `tool_search_call`、`tool_reference` → ロードされたツールの完全スキーマを構造化 JSON として運ぶ `tool_search_output` アイテム（`src/model/responses_request.rs`）。**デフォルトで有効（issue #286）**: シムは明らかにするたびにキャッシュ済みプロンプトプレフィックスを無効化してしまいます（明らかにされたツールを毎回 `tools` 配列へ追加し直す必要があり、shunt はその配列を毎ターン、クライアントの履歴から再導出するため）。そのためネイティブな、追記オンリーの `input` パスは、適用可能などこでも — ストックの OpenAI または ChatGPT/Codex Responses フレーバーが gpt-5.4+ モデルへルーティングする場合 — 今ではデフォルトになっています。ネイティブな形状は ChatGPT/Codex バックエンド（gpt-5.6、2026-07-13、[#86]）に対してライブプローブで検証済みであり、`tool_search` は公開 OpenAI Responses API の gpt-5.4+ モデル向けドキュメントにも記載されています。xAI/Grok のルートと gpt-5.2 以下のモデルは、フラグに関係なく静かに #43 のシムを維持します — エラーにはなりません。ネイティブパスに対応しているプロバイダーでシムへ戻したい場合は `tool_search = false` を設定してください。
...
- **A. tool-search のコンテキスト節約（追跡中: [#43]）。** **[#82]/[#286] でおおむね対処済み**: デフォルトで有効なプロバイダー別 `tool_search` フラグが、ストックの OpenAI または ChatGPT/Codex プロバイダーが gpt-5.4+ モデルへルーティングする場合に、tool search をテキストシムの代わりに Responses API のネイティブでクライアント実行の `tool_search` プロトコルへマッピングするようになりました（上の脚注 8 参照）。残るギャップは対象外のフレーバー/モデルです: xAI/Grok のルートと gpt-5.2 以下のモデルは依然としてシムへフォールバックします。シムは未ロードの遅延ツールを `tools` 配列から除外するため（`src/model/responses_request.rs:791-797`）、二度と明らかにされないツールについては実際にコンテキストを取り戻します — しかし Claude Code がツールを一つ明らかにすると、シムはその完全なスキーマを `tool_reference` テキストとしてレンダリングし（`:623-643`）、*さらに* そのツールを `tools` に再追加するため、明らかにするたびにスキーマが再送信され、それ以降キャッシュ済みプロンプトプレフィックスが無効化されます。Responses API はモデルが `tools` に見えているツールしか呼び出せないため、この二重送信はネイティブプロトコルなしには避けられません — 残存ギャップは、xAI/Grok と旧世代の OpenAI モデルがアップストリームで独自の `tool_search` 相当のサポートを備えて初めて解消されます。

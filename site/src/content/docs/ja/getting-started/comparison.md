...
- **C. Codex WS: ストリーム途中の失敗のフォールバック（解決済み: [#46]）。** 2 つの修正で解決しました。**[#93]** はチェックアウト段階で原因の 1 つを取り除きました。ハーフオープンなプール接続は、再利用ライブネスプローブ（ローカル書き込みだけでなく、時宜を得た `Pong` を要求）に通らなくなり、ターンのフレームを送信する前に新しいハンドシェイクへ差し替えられるため、古い接続がストリーム途中で切れることはなくなりました。続いて **[#46]** は、チェックアウトプローブではカバーできない残余区間（フレーム送信*後*・最初のトークン前にソケットが死ぬ send→first-token 区間）を塞ぎました。`open_ws_turn` が最初のイベントを先読み（peek）し、`commit_or_fallback` が最初のトークン前の転送エラーに対してターンを HTTP で再駆動します（`src/adapters/responses/mod.rs`）。最初のイベントがストリーミングされた*後*の失敗は真のストリーム途中の失敗であり、再開すると出力が重複するため、リプレイではなくクリーンな Anthropic `error` SSE イベントとして表面化します。`previous_response_id` によるターン途中の再開は意図的な非目標です（部分出力はすでにクライアントへコミット済み）。`tests/codex_websocket_fallback.rs` でカバーされています。

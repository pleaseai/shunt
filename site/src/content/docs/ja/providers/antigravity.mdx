...
（`fetchAvailableModels`）を読み — バックエンドごとに 10 分キャッシュ、時間制限つき、失敗時はそのまま
...
推測であってリクエストではありません。設定に書かれた認識できない `effort` はルール 1 のとおり書かれた
まま付与され、丸められません。ただし `-tiered` の経路では、バックエンドが知らない `thinkingLevel` を
拒否するため `medium` に畳まれます。すでにティアや `-tiered` で終わる id、カタログがそのまま公開している
id、そして Gemini 以外の id（`claude-sonnet-4-6`、`gpt-oss-120b-medium`）は書かれたまま送られます。

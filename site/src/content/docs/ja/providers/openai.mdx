...
- 一致した時点でアップストリームのリクエストを中断し、走らせたままにはしません。一致から中断までの
  間にアップストリームが生成したトークンは、引き続きアップストリーム側で課金されます。ストップ
  シーケンスを使う典型的な短い応答では、無視できる量です。

注意点がひとつあります。ターンがアップストリームの `response.completed` イベントより前に終わるため、
その使用量レポートは届きません。このようなターンの `message_delta.usage` には、shunt のローカルな
入力推定値と `output_tokens: 0` が入ります。

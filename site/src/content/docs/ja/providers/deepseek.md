---
title: DeepSeek
description: マッピングしたモデルを、DEEPSEEK_API_KEY を使って DeepSeek の Anthropic 互換エンドポイントへルーティングする。
---

**DeepSeek** は自社のモデルを **Anthropic 互換**エンドポイントで提供します — shunt は Claude Code の
Messages リクエストをそのまま転送し、DeepSeek の API キーを注入します。組み込みのプリセットはないため、
...
従来の `[providers.deepseek]` テーブル形式も引き続きサポートされます — ただし `[[upstreams]]` と
...
ません — `DEEPSEEK_API_KEY` が未設定の場合、`deepseek` へルーティングされた最初のリクエストが認証エラーを
返します。
...
| モデル ID | 備考 |
| :-- | :-- |
| `deepseek-v4-pro` | フロンティアティア |
| `deepseek-v4-flash` | 高速・軽量ティア |
...
レスポンスの `x-gateway-upstream` ヘッダーが `deepseek` を示すことを確認したら、
[Claude Code を shunt へ向け](/ja/guides/connect-claude-code/)てください。

## サブエージェントプラグイン

[`shunt-deepseek` プラグイン](https://github.com/pleaseai/shunt/tree/main/plugins/shunt-deepseek)は、
上記の各モデルにつき 1 つずつ、既製の Claude Code サブエージェントを出荷しています。

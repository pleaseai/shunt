...
After the chain is resolved, later elements may still be dropped when the request needs a feature that adapter is known not to serve (tools, images, structured output, explicit reasoning effort, or a `[1m]`/`[1M]` context hint). The primary is always kept. A `[1m]`/`[1M]` hint currently excludes every fallback because no adapter advertises a verified 1M path. Those exclusions are logged and counted as `capability_excluded`; they are not remembered as best failures.

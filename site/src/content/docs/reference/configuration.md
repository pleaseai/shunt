...
Backoff is exponential with randomized (full) jitter, capped at `max_backoff_ms`. A server-supplied `Retry-After` takes precedence (both the delta-seconds and HTTP-date forms are honored); decimal delta-seconds are accepted and rounded upward to the next whole second, while malformed or overlong values are ignored. Account cooldowns clamp a valid value to one hour, but retry-budget checks use the original requested duration, so a deadline beyond `max_backoff_ms` surfaces immediately rather than being shortened into a retry. Retry is **held off `count_tokens`** regardless of this setting. The `claude_oauth` / `chatgpt_oauth` / `kimi_oauth` account pools drive their own account-rotation failover and are unaffected by this table.

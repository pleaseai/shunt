...
| `tool_search` | `true` (default) \| `false` | Use the native client-executed `tool_search` protocol for Claude Code's tool search on stock OpenAI / ChatGPT-Codex flavors on GPT-5.4+ models; other flavors/models keep the text-based shim regardless. Set to `false` to force the shim on a provider that does support the native path. See [Codex → Tool search](/guides/codex/#native-protocol). |

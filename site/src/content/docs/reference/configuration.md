...
| `kind` | without a preset | `anthropic`, `responses`, `cursor`, `gemini`, `antigravity`, or `antigravity_cli`. The last three have no entry in the preset table below — the built-in `[providers.*]` tables of the same names are the separate legacy mechanism, not presets — so an ordered upstream on either must set `kind` explicitly. |

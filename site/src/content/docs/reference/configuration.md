...
Independent of origin, each retained slot is also checked by the value it actually holds: `authorization` and `x-api-key` are each cleared only when that slot's own value verifies as shunt's own `[server.gateway]` JWT — an `apiKeyHelper` fills both slots with the same value, so the JWT can land in either or both. A slot holding a genuine upstream credential is forwarded even when the other slot holds the gateway JWT; only the JWT-bearing slot is cleared.

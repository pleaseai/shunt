...
For a multi-entry model map, shunt filters the declared upstream sequence to the names in the map. It advances after an upstream status `429`, `401`, `403`, `404`, or any `5xx`, and after a failure before upstream response headers arrive. Gateway-local errors that do not represent an upstream attempt, such as auth misconfiguration or adapter-owned validation/header construction errors, return immediately so failover does not mask the configuration problem. There is no failover after `2xx` headers have been returned, including a later streaming-body failure. The Responses adapter's streaming paths commit the response before any upstream byte; a chain of `Anthropic`/`Responses` elements without the websocket transport then runs its failover inside the committed stream — pre-header failures, timeouts, and advance-status errors retry the next upstream before the synthetic start goes out — while a route that cannot advance (a websocket element, another adapter kind, an exhausted account pool, or a terminal non-2xx) surfaces the failure as one terminal SSE `error` event on the committed stream. On that committed path `x-gateway-upstream` and `x-gateway-upstream-model` name the routed (first) upstream, not the eventual winner — the headers go out with the commit — while the request metrics and stream attribution follow the winner once the stream knows it.

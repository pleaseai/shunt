...
The spend-limit routes exist only when [`[server.gateway.admin]`](/reference/configuration/#servergatewayadmin-optional) was configured at boot. Send an `x-api-key` from `write_keys_env` for all operations; a key from `read_keys_env` can use GET only. `POST` accepts `user` and `organization` scopes, a `daily`/`weekly`/`monthly` period, and an `amount` that is either a whole-number string of USD cents or `null`. It upserts by `(scope, period)`. List pagination accepts `limit` (1–1000, default 20), `after_id`, `before_id`, and `scope_type`; the two cursors are mutually exclusive. Every response includes `request-id`, and errors use the Anthropic Admin API error envelope. Caps and mutation audit records persist together in the configured versioned JSON state file. Stage 1 does not expose `/effective` or `/audit` and does not enforce caps on inference requests.

...
| `GET` | `/admin/api/routes` | The routing table the dashboard renders — the same body `GET /routes` above returns, built from the same snapshot so the operator's view and the table a client resolves against cannot drift. It is registered separately rather than redirected because [`[server.admin]`](/reference/configuration/#serveradmin-optional) may bind the admin surface to its own listener, where `/routes` is not registered. Unlike its unauthenticated public counterpart this one requires the admin credential, so leaving `/routes` open does not widen what that credential gates |

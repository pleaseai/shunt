...
| `GET` | `/routes` | shunt-native route discovery — returns the configured `[[routes]]` table verbatim (model → provider/upstream_model/effort mapping, including claude-prefixed discovery aliases); distinct from `/v1/models`, which serves the narrower Anthropic-protocol discovery response (`id`, `display_name`, and upstream model metadata). A `routers` array is added when at least one `[[models]]` entry configures a [stage router](/guides/stage-router/), naming each router's model id, its two tier targets, and the tier its picker falls back to when no signal decides |
...
| `GET` | `/admin/api/session` | The per-session values the admin SPA needs before it can render: the session's `csrf` token, `expiry_buffer_ms` (`claude::auth::EXPIRY_BUFFER` in milliseconds, the boundary at which a setup token stops being usable), and `access` — `read` or `write`, the tier the session authenticates with, which is what the dashboard renders its write affordances from. It also carries `hide_observed`, the boolean value of `[server.admin].hide_observed`, from which the dashboard decides whether to request `/admin/api/observed` at all. A header-credential caller is CSRF-exempt and receives an empty `csrf`. Returning the token over a `GET` is safe because no CORS layer exists on this surface, so a cross-origin page can send the request but cannot read the reply |

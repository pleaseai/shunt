...
| `GET` | `/routes` | shunt-native route discovery — returns the configured `[[routes]]` table verbatim (model → provider/upstream_model/effort mapping, including claude-prefixed discovery aliases); distinct from `/v1/models`, which serves the narrower Anthropic-protocol discovery response (`id`, `display_name`, and upstream model metadata). A `routers` array is added when at least one `[[models]]` entry configures a `[models.router]` table, naming each router's model id, its `algorithm` (the configured `type`), and every `targets` id it can route to; a [stage router](/guides/stage-router/) or `auto` entry additionally reports its two tier targets and the tier its picker falls back to when no signal decides |

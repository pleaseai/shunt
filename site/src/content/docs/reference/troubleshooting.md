...
| 429 on Anthropic passthrough models | Check `rate_limit_kind` in the gateway log. `quota` (has `retry-after` / `anthropic-ratelimit-*` headers) is a real rate limit — back off or reduce parallel load. `client-shape-rejection` (neither header, body just `"Error"`) means api.anthropic.com rejected a subscription-OAuth request that doesn't look like Claude Code — non-Claude-Code clients must use an API key instead of an OAuth token. A burst of these can also knock out Claude Code's auto-mode classifier ("model temporarily unavailable"). |

...
| `ANTHROPIC_DEFAULT_{OPUS,SONNET,HAIKU,FABLE}_MODEL` | Repoints a built-in tier alias at a shunt-routed id; `_NAME` and `_DESCRIPTION` control how it renders in the picker. `opusplan` has no variable of its own — it follows the Opus id in Plan Mode and the Sonnet id outside it. The Sonnet variable reaches further than the picker: auto mode's permission classifier picks its model from the Sonnet tier alias (measured on Claude Code 2.1.226), so repointing `ANTHROPIC_DEFAULT_SONNET_MODEL` moves every permission check too. To move the classifier alone, set `classifier_model` on the provider gateway-side instead — see [Anthropic → Auto-mode classifier](/providers/anthropic/#auto-mode-classifier). See [Model Aliases](/guides/model-aliases/#alias-resolution). |
...
| `CLAUDE_CODE_GATEWAY_HINT_HEADERS=1` | Sends the `x-claude-code-*` request-hint headers that describe each turn: the request class (`main`, `subagent`, `workflow`, `compaction`, `auxiliary`), the agent type, and a one-shot flag on the first turn after a context compaction. **The variable wins when it is set**; unset, these headers are sent only when the base URL is a first-party Anthropic host, so behind shunt they are off. shunt reads them into its [stage router](/guides/stage-router/#session-pinning) context: the class decides which turns count as delegated work, taking precedence over `x-claude-code-agent-id` (which is **not** gated by this variable and is sent on every delegated turn either way, so sub-agent tier pins already work without it), and the compaction flag escalates that turn to the capable tier and stays latched on the session's pin until the pin expires. The agent type is what a [`[models.subagents]`](/reference/configuration/#modelssubagents-optional) `by_type` map keys on, so per-type targets also need this variable. |

...
| `CLAUDE_CODE_ALWAYS_ENABLE_EFFORT=1` | Forces the `effort` parameter for model ids the client would not otherwise send it for. Set it for a custom gateway id, which otherwise falls back to `medium` — see [Effort & Context](/guides/effort-and-context/#reasoning-effort). Verified against v2.1.224: the flag is not absolute — it cannot re-enable effort for a model the client has already marked effort-unsupported. |

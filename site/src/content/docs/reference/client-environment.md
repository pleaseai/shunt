...
| `API_FORCE_IDLE_TIMEOUT` | Controls whether the HTTP client's idle timeout applies to Anthropic API requests. Verified against v2.1.224: by default that timeout is disabled while Claude Code's own stream-idle watchdog is active; `1` forces it to stay enabled, and an explicit `0` disables it even without the watchdog. Worth pinning explicitly if a gateway hop changes the idle characteristics of long streams. |

...
Source the printed file path before starting Shunt. Cursor and Command Code use their existing token variables. API keys use `OPENAI_API_KEY`, `XAI_API_KEY`, `SHUNT_COMMANDCODE_API_KEY`, or `SHUNT_IMPORTED_<PROVIDER>_API_KEY` (uppercase, hyphens become underscores); match this variable in your existing `api_key_env`. No config or routing is generated. Snapshots are not refreshed; re-import when access tokens expire. Importing a key does not admit an unsupported provider such as OpenCode Go.

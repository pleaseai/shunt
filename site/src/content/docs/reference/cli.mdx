...
`[providers.antigravity] base_url` when the config file supplies one — the same host the request
path uses — and falls back to the default Code Assist backend when no config is readable.

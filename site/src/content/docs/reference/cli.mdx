...
credential is still saved and discovery is retried on the first request. Discovery addresses
`[providers.antigravity] base_url` — the same host the request path uses — when that provider is the
Antigravity upstream (`kind = "antigravity"` with `auth = "antigravity_oauth"`), whose `base_url`
config validation has pinned to the Code Assist host or loopback. Any other table parked under that
name, and an unreadable config, fall back to the default Code Assist backend rather than send the
subscription token to an unvetted host.

...
| tool-search / `defer_loading` / `tool_reference` 처리 | ◐ (스톡 OpenAI/Codex의 gpt-5.4+에서 네이티브가 기본값⁸; 그 외는 심) | ○⁵ | ◐ (업스트림) / ● (포크) | ○ | ○ |
...
⁸ **[#82]**는 프로바이더별 `tool_search` 플래그(`src/config.rs:1136-1149,2792-2805`)를 추가해 Claude Code의 도구 검색을 스키마를 텍스트로 접어 넣는 대신 OpenAI Responses API 자체의 네이티브 클라이언트 실행 `tool_search` 프로토콜에 매핑합니다 — `ToolSearch` → `tool_search`, 그 `tool_use` → `tool_search_call`, `tool_reference` → 로드된 도구들의 전체 스키마를 구조화된 JSON으로 담는 `tool_search_output` 항목(`src/model/responses_request.rs`). **기본으로 켜짐(issue #286)**: shim은 드러날 때마다 캐시된 프롬프트 프리픽스를 무효화합니다(드러난 도구를 매번 `tools` 배열에 다시 더해야 하는데, shunt는 이 배열을 매 턴 클라이언트 히스토리로부터 다시 도출하기 때문입니다) — 그래서 이제는 적용 가능한 모든 곳, 즉 스톡 OpenAI 또는 ChatGPT/Codex Responses 계열이 gpt-5.4+ 모델로 라우팅되는 경우 네이티브 append-only `input` 경로가 기본값입니다. 네이티브 형태는 2026-07-13에 gpt-5.6에서 ChatGPT/Codex 백엔드를 대상으로 라이브 프로브로 검증되었습니다; `tool_search`는 gpt-5.4+ 모델에 대해 공개 OpenAI Responses API 문서에도 실려 있습니다. xAI/Grok 라우트와 gpt-5.2 이하 모델은 무관하게 #43 shim을 유지합니다 — 조용히, 오류 없이. 네이티브 경로를 지원하는 프로바이더에서 shim으로 되돌리려면 `tool_search = false`로 설정하세요.
...
- **A. tool-search 컨텍스트 절약 (추적 중: [#43]).** **[#82]/[#286]으로 대부분 해결**: 프로바이더별 `tool_search` 플래그가 이제 기본으로 켜져, 스톡 OpenAI 또는 ChatGPT/Codex 프로바이더가 gpt-5.4+ 모델로 라우팅될 때 도구 검색을 텍스트 shim 대신 Responses API의 네이티브 클라이언트 실행 `tool_search` 프로토콜에 매핑합니다(위 각주 8 참고). 남은 격차는 자격을 갖추지 못한 계열/모델입니다: xAI/Grok 라우트와 gpt-5.2 이하 모델은 여전히 shim으로 폴백하는데, 이 shim은 아직 로드되지 않은 지연 도구를 `tools` 배열에서 제외합니다(`src/model/responses_request.rs:791-797`) — 그래서 끝내 드러나지 않는 도구에 대해서는 실제로 컨텍스트를 회수합니다 — 다만 Claude Code가 도구 하나를 드러내면 shim은 그 전체 스키마를 `tool_reference` 텍스트로 렌더링하는 동시에(`:623-643`) 그 도구를 `tools`에 다시 추가하므로, 노출 때마다 스키마가 다시 전송되고 그 지점부터 캐시된 프롬프트 프리픽스가 무효화됩니다. Responses API는 모델이 `tools`에서 보이는 도구만 호출하게 하므로, 네이티브 프로토콜 없이는 이 이중 전송을 피할 수 없습니다 — 이 잔여 격차는 xAI/Grok과 구형 OpenAI 모델이 업스트림에서 자체적으로 `tool_search`에 준하는 지원을 얻어야만 닫힙니다.

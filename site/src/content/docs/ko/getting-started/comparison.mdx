...
shunt는 **스펙을 준수하는 Claude Code LLM 게이트웨이**입니다: Claude Code의 공식 `ANTHROPIC_BASE_URL` 게이트웨이 계약(`/v1/messages`, `/v1/models` 디스커버리, 어트리뷰션/헤더 패스스루)을 구현하고, **`model` id 단위의 선택적** 우회를 수행합니다 — 메인 세션은 Claude에 유지하고, 지정한 모델만 다른 프로바이더(ChatGPT/Codex, OpenAI, xAI)로 돌립니다. 매핑된 모델에 대해서는 Anthropic Messages ⇄ OpenAI Responses API를 변환하고, 그 외는 모두 Anthropic으로 그대로 패스스루합니다. 라우팅은 요청의 `model` id로 합니다 — 프롬프트 형태 핑거프린팅이 없습니다(`README.md:104-131`). 유일한 예외는 옵트인이며 프롬프트 텍스트를 전혀 읽지 않습니다. `type = "stage_router"`를 선언한 [`[models.router]`](/ko/guides/stage-router/) 항목은 구조화된 tool-result 메타데이터로 두 티어 중 하나를 고르는데, 이는 다음 Claude Code 릴리스가 다시 쓰는 프롬프트가 아니라 안정된 와이어 계약입니다. 같은 테이블의 나머지 타입 — `auto`, `random`, `noop` — 은 요청 본문을 아예 읽지 않습니다.
...
⁵ **raine/ccp 소스를 읽어 확인함**(`fe80a6b`, 2026-07-11): tool-search 처리가 존재하지 않습니다(`defer_loading` / `tool_reference` / `tool_search` / `advanced-tool-use`에 대한 매치 0건). 도구는 `{name, description, parameters}`로 화이트리스트 재구성되므로(`src/providers/codex/translate/request.rs:476-494`) `defer_loading:true`는 조용히 버려집니다 — 400은 아니지만 컨텍스트 절약도 없음; ToolSearch 결과의 `tool_reference` 블록은, 알려진 참조에 대해 shunt가 렌더링하는 전체 스키마(`src/model/responses_request.rs:637-661`) 대신 `[unsupported content block omitted: tool_reference]`로 렌더링됩니다(`request.rs:836-842`) — shunt의 `"Loaded tool: X"`는 인식되지 않은 참조에 대한 폴백일 뿐입니다. 그래서 ○(shunt의 ◐ 대비): raine/ccp에 대해 `ENABLE_TOOL_SEARCH`를 강제로 켜면 디스커버리 루프 결과가 플레이스홀더로 격하됩니다. 기본적으로는 Claude Code 자체 게이트가 퍼스트파티가 아닌 base URL 뒤에서 tool search를 꺼 두므로 이 문제는 잠재된 채로 남습니다.
...
- **A. tool-search 컨텍스트 절약 (추적 중: [#43]).** **[#82]/[#286]으로 대부분 해결, [#289]로 범위 조정**: 프로바이더별 `tool_search` 플래그는 기본값("auto")에서 이미 지원이 확인된 호스트 — ChatGPT/Codex 백엔드와 `api.openai.com` — 가 gpt-5.4+ 모델로 라우팅될 때만 도구 검색을 텍스트 shim 대신 Responses API의 네이티브 클라이언트 실행 `tool_search` 프로토콜에 매핑합니다(위 각주 8 참고); 그 외 OpenAI 호환 엔드포인트는 명시적인 `tool_search = true` 옵트인이 필요합니다. 남은 격차는 자격을 갖추지 못한 계열/모델/호스트입니다: xAI/Grok 라우트, gpt-5.2 이하 모델, 그리고 아직 옵트인하지 않은 커스텀 OpenAI 호환 엔드포인트는 모두 여전히 shim으로 폴백하는데, 이 shim은 아직 로드되지 않은 지연 도구를 `tools` 배열에서 제외합니다(`src/model/responses_request.rs:819-830`) — 그래서 끝내 드러나지 않는 도구에 대해서는 실제로 컨텍스트를 회수합니다 — 다만 Claude Code가 도구 하나를 드러내면 shim은 그 전체 스키마를 `tool_reference` 텍스트로 렌더링하는 동시에(`:637-661`) 그 도구를 `tools`에 다시 추가하므로, 노출 때마다 스키마가 다시 전송되고 그 지점부터 캐시된 프롬프트 프리픽스가 무효화됩니다. Responses API는 모델이 `tools`에서 보이는 도구만 호출하게 하므로, 네이티브 프로토콜 없이는 이 이중 전송을 피할 수 없습니다 — 이 잔여 격차는 xAI/Grok과 구형 OpenAI 모델이 업스트림에서 자체적으로 `tool_search`에 준하는 지원을 얻거나, 더 많은 커스텀 엔드포인트가 검증되어 옵트인해야만 닫힙니다.

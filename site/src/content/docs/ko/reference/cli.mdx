...
shunt는 배포의 `/.well-known/oauth-authorization-server` 문서를 읽어 거기서 광고된 엔드포인트를 사용하고, 확인용 URL과 짧은 사용자 코드를 출력한 뒤 브라우저를 열고, 게이트웨이의 `/device` 페이지에서 승인할 때까지 폴링합니다. discovery가 `404`를 반환한다면 대개 그 배포에 `[server.gateway]` 섹션이 없다는 뜻입니다. 루프백이 아닌 평문 `http://` URL도 허용되지만 경고가 출력됩니다 — device code와 refresh token이 암호화되지 않은 채 전송되기 때문입니다.

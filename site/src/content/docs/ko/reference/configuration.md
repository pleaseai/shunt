...
| `kind` | preset이 없는 경우 | `anthropic`, `responses`, `cursor`, `gemini`, `antigravity`, `antigravity_cli` 중 하나. 뒤의 두 종류는 아래 preset 표에 항목이 없으므로 — 같은 이름의 내장 `[providers.*]` 테이블은 preset이 아니라 별개의 레거시 방식입니다 — 정렬 업스트림에서 `kind`를 직접 지정해야 합니다. |

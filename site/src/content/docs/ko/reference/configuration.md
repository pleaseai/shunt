...
체인이 구성된 뒤에도, 요청이 해당 어댑터가 지원하지 않는 것으로 알려진 기능(tools, 이미지, structured output, 명시적 reasoning effort, 또는 `[1m]`/`[1M]` 컨텍스트 힌트)을 필요로 하면 이후 항목은 제외됩니다. primary는 항상 유지됩니다. `[1m]`/`[1M]` 힌트는 검증된 1M 경로를 광고하는 어댑터가 없어 현재 모든 폴백을 제외합니다. 제외는 `capability_excluded`로 기록되며 최종 실패로 기억되지 않습니다.

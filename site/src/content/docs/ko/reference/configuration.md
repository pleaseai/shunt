...
| `kind` | preset이 없는 경우 | `anthropic`, `responses`, `cursor`, `openai_chat`, `gemini`, `antigravity`, `antigravity_cli` 중 하나. `openai_chat`은 텍스트 전용 Anthropic Messages → OpenAI Chat Completions 어댑터이며 도구 변환은 아직 지원하지 않아 거부합니다. 뒤의 네 종류는 정렬 업스트림에서 `kind`를 직접 지정해야 합니다. |
...
| 키 | 값 | 의미 |
| :-- | :-- | :-- |
| `kind` | `anthropic` \| `responses` \| `cursor` \| `openai_chat` \| `gemini` \| `antigravity` \| `antigravity_cli` | 업스트림 프로토콜 / 어댑터. `openai_chat` = 텍스트 전용 Anthropic Messages → OpenAI Chat Completions 변환(스트리밍 사용량 트레일러 지원, 도구 변환은 미지원); `anthropic` = Messages API; `responses` = Anthropic Messages를 OpenAI Responses API로 변환; `cursor` = 네이티브 Cursor ConnectRPC/protobuf AgentService 어댑터; `gemini` = Gemini Code Assist 변환; `antigravity` = Google Antigravity HTTP 백엔드; `antigravity_cli` = **더 이상 사용되지 않음** — 로컬 CLI를 실행합니다. |

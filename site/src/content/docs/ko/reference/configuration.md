...
조건이 하나 더 붙습니다. 판정 호출은 자기 라우트가 주입하는 자격 증명으로 실행되므로,
타깃이 passthrough 업스트림으로 해석되면 실행할 자격 증명이 없어 시작 오류가 됩니다.
...
전에도 부르지 않습니다. 인바운드 인증은 요청된 id에 더해 이 항목이 지정할 수 있는 모든
타깃과 판정 모델을, 각각의 페일오버 체인 전체까지 포함해 대상으로 삼습니다. 그래서
passthrough 응답 타깃에 자격 증명을 주입하는 판정 모델이 붙으면 클라이언트 자격 증명이
필요해지고, 인증에 실패했거나 정책이 거부한 요청은 판정 호출을 한 번도 만들지 않습니다.
...
| `gated_idle_ms` | `60000` | 보관하는 턴에서 본문 청크 사이의 최대 간격. SSE ping 프레임은 이 타이머를 되돌리지 않습니다 |
...
`confidence_threshold`와 똑같이 범위를 검사하고, 판정 타깃은 자격 증명을 주입하는
라우트로 해석되어야 합니다 — 실효 체인에 passthrough 업스트림이 들어 있는 타깃은 판정을
실행할 자격 증명이 없으므로 시작 오류입니다. [호출당 한도](#호출당-한도) 여섯 개 중

...
| `tool_search` | `true`(기본) \| `false` | 스톡 OpenAI / ChatGPT-Codex 계열의 GPT-5.4+ 모델에서 Claude Code의 도구 검색에 네이티브 클라이언트 실행 `tool_search` 프로토콜을 사용합니다; 그 외 계열/모델은 항상 텍스트 기반 shim을 유지합니다. 네이티브 경로를 지원하는 프로바이더에서 shim을 강제하려면 `false`로 설정하세요. [Codex → 도구 검색](/ko/guides/codex/#네이티브-프로토콜)을 참고하세요. |

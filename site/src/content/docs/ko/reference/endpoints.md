...
| `GET` | `/admin/api/routes` | 대시보드가 렌더링하는 라우팅 표 — 위의 `GET /routes`가 반환하는 것과 같은 본문이며, 같은 스냅샷에서 만들어지므로 운영자가 보는 표와 클라이언트가 실제로 해석하는 표가 어긋날 수 없습니다. 리다이렉트가 아니라 별도로 등록하는 이유는 [`[server.admin]`](/reference/configuration/#serveradmin-선택)이 관리자 표면을 자체 리스너에 바인딩할 수 있고, 그 리스너에는 `/routes`가 등록되지 않기 때문입니다. 인증이 없는 공개 엔드포인트와 달리 이쪽은 관리자 자격 증명을 요구하므로, `/routes`를 열어 두어도 그 자격 증명이 가리는 범위가 넓어지지 않습니다 |

...
| `GET` | `/admin/api/session` | 관리자 SPA가 렌더링 전에 필요한 세션별 값: 세션의 `csrf` 토큰, `expiry_buffer_ms`(`claude::auth::EXPIRY_BUFFER`의 밀리초 값으로, setup token이 사용 불가로 바뀌는 경계), 그리고 `access` — 이 세션이 인증하는 등급으로 `read` 또는 `write`이며, 대시보드는 이 값으로 쓰기 동작의 노출 여부를 결정한다. 또한 `[server.admin].hide_observed`의 불리언 값인 `hide_observed`도 담으며, 대시보드는 이 값으로 `/admin/api/observed`를 아예 요청할지 여부를 결정한다. 헤더 자격 증명 호출자는 CSRF 면제 대상이라 빈 `csrf`를 받는다. 이 표면에는 CORS 레이어가 없어 교차 출처 페이지가 요청은 보낼 수 있어도 응답은 읽지 못하므로, `GET`으로 토큰을 반환해도 안전하다 |

...
| 도구 검색은 동작하지만 컨텍스트를 회수하지 못함(여전히 텍스트 shim의 캐시 무효화 비용을 지불 중) | 네이티브 `tool_search`가 기본값이므로 계열/모델 게이트를 확인하세요: 스톡 OpenAI 또는 ChatGPT/Codex 계열 프로바이더가 gpt-5.4 이상 모델로 라우팅되어야 합니다. 지원되지 않는 계열/모델은 조용히 텍스트 shim을 유지합니다. 두 게이트를 모두 통과했다면 `tool_search = false`를 설정하지 않았는지 확인하세요 — [ChatGPT / Codex → 도구 검색 → 네이티브 프로토콜](/ko/guides/codex/#네이티브-프로토콜)을 참고하세요. |
...
| 공유 게이트웨이에서 매핑된 모델에 401 | 누락/유효하지 않은 클라이언트 토큰 — `ANTHROPIC_AUTH_TOKEN=<token>`(`Authorization: Bearer`로 수락, 풀 전용 게이트웨이) 또는 `ANTHROPIC_CUSTOM_HEADERS="x-shunt-token: <token>"`(패스스루 모델이 섞여 있으면 필수)을 설정하세요; [게이트웨이 공유](/ko/guides/shared-gateway/#인바운드-클라이언트-토큰)를 참고하세요. |

...
- 기본 전환 임계값은 `0.98`입니다. 5시간 status가 `rejected`이거나, 적용되는 주간 status가 `rejected`이거나, 공유 5시간 사용률이 해당 임계값에 도달했거나, 적용되는 주간 사용률이 해당 임계값에 도달하면 그 계정은 쿼터에 근접한 것입니다. Fable 모델의 경우 적용되는 주간 status는 `7d_oi` 사용률이 있으면 `7d_oi`이고, 없으면 공유 `7d`입니다. 집계 status는 창별 status가 하나도 기록되지 않았을 때만 쓰이는 호환용 폴백입니다. 임계값은 계정별로(위의 `threshold*` 필드) 또는 풀 전체로(자세한 내용은 [선택 튜닝](#선택-튜닝-serverpool) 참고) 낮출 수 있습니다.
...
| Fable 요청에서 `anthropic-ratelimit-unified-7d_oi-status: rejected`만 있는 429 | Fable 쿼터 소진: 숫자 `retry-after`로 Fable 요청에만 적용되는 쿨다운(기본 60초, 1–3600초로 클램프)을 건 뒤 로테이션합니다. non-Fable 요청은 계속 그 계정을 쓸 수 있습니다. |

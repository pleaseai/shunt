...
- 헤더가 없거나 빈 값으로 오면: 프로바이더마다 자체 라운드 로빈 카운터를 사용합니다. 빈 값은 하나의 대화가 아니므로, 고정된 계정 하나로 해시되지 않고 이 로테이션을 함께 씁니다.
...
- 관리자 풀 엔드포인트(`GET /admin/api/pool`)는 각 계정의 `priority`, `disabled` 플래그를 보고하며, `[server.pool]`이 구성되어 있으면 현재 여유(headroom) 예측치를 초 단위로 함께 보고합니다; 대시보드의 상태 열은 비활성화된 계정을 표시합니다.

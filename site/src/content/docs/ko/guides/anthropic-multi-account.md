...
| `uuid` | 아니요 | 기존 `metadata.user_id.account_uuid`를 다시 쓰기 위한, 선택된 계정의 Anthropic UUID이자, 풀에서 별칭을 통합하는 데 쓰이는 안정적인 아이덴티티. 이름만 있는 항목(스토어 스캔으로 해석됨)은 선택이 일어나기 전에 스토어의 `shuntAccountUuid`로부터 자동으로 채워지지만, `credentials` 또는 `token_env`로 설정된 항목은 일치하는 `uuid`를 명시적으로 설정한 경우에만 다른 별칭과 통합됩니다. |

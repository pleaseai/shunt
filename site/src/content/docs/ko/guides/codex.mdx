...
이 플래그를 켜면 shunt는 기본적으로 도구 검색을 OpenAI Responses API 자체의 **네이티브 클라이언트 실행
`tool_search`** 프로토콜에 매핑합니다: Claude Code의 `ToolSearch` 도구는 `tool_search`(`execution:
...
- 기본 동작을 쓰는 데는 `shunt.toml` 변경이 필요 없습니다 — 순수하게 `ENABLE_TOOL_SEARCH` 수준의
  Claude Code 환경 변수입니다.
- `codex`(ChatGPT) 및 `openai`(스톡 Responses) 프로바이더에 적용됩니다.
...
`tool_search`는 `[providers.<name>]` 아래에서 기본적으로 `true`입니다. 네이티브 형태는 2026-07-13에
gpt-5.6에서 ChatGPT/Codex 백엔드를 대상으로 라이브 프로브로 검증되었으며
([pleaseai/shunt#86](https://github.com/pleaseai/shunt/pull/86)), `tool_search`는 gpt-5.4+
모델에 대해 공개 OpenAI Responses API 문서에도 실려 있습니다. 네이티브 경로를 지원하는 프로바이더에서
텍스트 shim으로 되돌리려면 `tool_search = false`로 설정하세요:
...
| 도구 검색이 지연만 할 뿐 컨텍스트를 실제로 회수하지 않음(여전히 shim의 캐시 무효화 비용을 지불 중) | 계열/모델 게이트를 확인하세요 — 네이티브 `tool_search`는 스톡 OpenAI/ChatGPT-Codex 계열과 gpt-5.4 이상 모델이 필요하며, 지원되지 않는 조합은 조용히 shim으로 폴백합니다. 두 게이트를 모두 통과했다면 `tool_search = false`를 설정하지 않았는지 확인하세요 — 위의 [도구 검색 → 네이티브 프로토콜](#네이티브-프로토콜)을 참고하세요. |

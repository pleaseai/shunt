...
게이트웨이 로그인 세션에는 Claude 앱 게이트웨이의 기능 트레이드오프가 따릅니다: WebSearch가 비활성화되고, 퍼스트파티 전용 beta 헤더와 1시간 cache-TTL beta가 생략되며, 로그인에 브라우저가 필요합니다. 개인용이나 단일 사용자 설정이라면 [`ANTHROPIC_BASE_URL`](/ko/guides/connect-claude-code/)과 선택적 [`[server.auth]`](/ko/guides/shared-gateway/)를 계속 사용하세요.
...
Claude Code와 사용자 브라우저가 도달할 수 있는 공개 URL을 추가하세요:

```toml
[server.gateway]
public_url = "https://gateway.example.com"
jwt_secret_env = "SHUNT_GATEWAY_JWT_SECRET" # 기본값
users_env = "SHUNT_GATEWAY_USERS"            # 기본값
token_ttl_seconds = 3600                      # 기본값
trust_forwarded_for = false                   # 기본값
# state_path = "~/.shunt/gateway-sessions.json"  # 기본값; ""는 메모리 전용 세션
```

`public_url`이 순수 HTTPS origin이 아니거나(`http`는 루프백에서만 허용됨), `token_ttl_seconds`가 0이거나, 서명 시크릿이 32바이트보다 짧거나, 유효한 사용자 목록과 유효한 외부 IdP 중 어느 것도 구성되지 않았으면 시작은 닫힌 채로 실패(fail closed)합니다. 정적 사용자의 시크릿에는 `:`가 포함될 수 있습니다 — 첫 번째 콜론만 이메일과 시크릿을 구분하기 때문입니다.
...
비어 있지 않은 텔레메트리 목적지 목록은 표준 Claude Code OTLP 환경 값 여섯 개를
푸시합니다. 정책 `env` 키는 주입된 기본값을 오버라이드합니다:
...
이 구성은 현재 관리형 환경 푸시를 게이팅합니다. 인증된 OTLP
ingest/relay 라우트는 M-C(#189)에서 별도로 제공됩니다.
...
관리형 정책, `ETag` 캐싱, 텔레메트리 환경 푸시, 서버 측 모델 allow-list
적용은 위에서 설명했습니다. 인증된 인바운드 OTLP 텔레메트리는 별도의
M-C 후속 작업으로 남아 있습니다.

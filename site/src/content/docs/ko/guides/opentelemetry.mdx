...
| `shunt.latency` | 히스토그램(ms) | `provider`, `model`, `http.response.status_code` | 스트림은 헤더 지연 시간, 그 외에는 전체 지연 시간. 커밋형 스트리밍 응답은 헤더가 업스트림 응답보다 먼저 커밋되므로 디스패치부터 시도 분류까지를 측정합니다. |

...
| `shunt.upstream.status` | 게이지 | `provider` | 관측된 [`[server.status]`](/reference/configuration/#serverstatus-optional) Statuspage severity: `0` none, `1` minor, `2` major, `3` critical. 현재 `unknown`(신호 없음) 상태인 provider는 `0`으로 보고되는 대신 이 시리즈에서 완전히 제외됩니다. |

...
| `shunt.stage_router.decisions` | 카운터 | `model`, `tier`, `source` | [스테이지 라우터](/ko/guides/stage-router/)의 티어 결정. 요청된 model id, 선택된 티어(`capable`/`efficient`), 그렇게 고른 이유별로 집계합니다. `count_tokens` 프로브는 제외됩니다. |

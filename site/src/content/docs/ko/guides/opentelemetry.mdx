...
| `shunt.router.decisions` | 카운터 | `model`, `algorithm`, `target`, `source` | 모든 [`[models.router]`](/ko/reference/configuration/) `type`의 라우팅 결정. 라우터의 model id(매칭된 형태라 `[1m]` 힌트는 이미 제거됨), 설정된 `type`, 결정이 해석된 model id, 그렇게 고른 이유별로 집계합니다. `count_tokens` 프로브는 제외됩니다. |

...
세 번째 설정인 `profile_dir`은 항목마다 자기 `HOME`을 줍니다. `agy`는 크리덴셜을 포함한 상태 트리 전체를 `HOME`으로
해석하므로, 설정하지 않으면 모든 항목이 게이트웨이 호스트의 단일 ambient `~/.gemini` 프로필을 공유하고 결국 한 Google
계정만 서비스합니다. 항목별로 디렉터리를 지정하면 각자 자기 계정에 로그인하고 여러 계정을 동시에 풀링할 수 있습니다.
디렉터리는 필요할 때 생성되고 절대 경로로 정규화되며, 맨 앞의 `~`는 확장됩니다. 각 디렉터리마다 `agy` 로그인을 한 번씩
해 줘야 합니다. 이 설정을 쓴 항목에 한해 자식 프로세스의 `GOOGLE_*` / `GEMINI_*` 환경 변수가 제거되므로, 호스트의 설정이
어느 계정 — 그리고 누구의 청구서 — 이 요청을 처리할지 조용히 바꿔 버릴 수 없습니다. 지정하지 않으면(기본값) 기존 동작
그대로입니다. 정렬 업스트림(`[[upstreams]]`)에서도 같은 설정을 쓸 수 있습니다.
...
ChatGPT 계정 Codex 백엔드는 `gpt-*-codex` 슬러그를 **거부**합니다 — 계정에 실시간으로 부여된(live-entitled) 슬러그만 받아들입니다. 권위 있는 카탈로그는 openai/codex의 [`models.json`](https://github.com/openai/codex/blob/main/codex-rs/models-manager/models.json)입니다. 현재 슬러그는 `gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`(최신), `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`(프런티어)와 `gpt-5.5` / `gpt-5.4`입니다. 오래된 계정은 이전 슬러그만 부여받았을 수 있습니다. 라우트에서 `upstream_model`을 사용하면 임의의 별칭을 부여된 슬러그로 매핑할 수 있습니다.
...
| [OpenCode Zen](/ko/providers/opencode/) | `https://opencode.ai/zen` | `claude-fable-5-1`, `gpt-6-astra` — 큐레이션된 크로스 벤더 카탈로그 (`x-api-key` 읽음) |
...
| :-- | :-- | :-- |
| `shunt-codex` | `gpt-6-sol`, `gpt-6-luna`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` | `codex` (ChatGPT 구독) |

...
스스로 해결하므로, 요청의 `tools` 배열은 전달되지 않고 응답에도 `tool_use` 블록이 담기지 않습니다. 도구 루프를 직접
구동해야 하는 클라이언트는 `gemini` 프로바이더를 사용하세요.
...
이런 설정이 전혀 필요 없는 위의 네이티브 `antigravity` HTTP 프로바이더를 사용하는 편이 좋습니다. 전체 내용은
[더 이상 사용되지 않는 antigravity-cli 전송](/guides/providers/#the-deprecated-antigravity-cli-transport)을 참고하세요.
...
| [Kimi Code(구독, OAuth)](/ko/providers/kimi/#kimi-code-oauth-subscription) | `https://api.kimi.com/coding` | 구독이 노출하는 모델 ID 사용 |
...
`kimi_oauth`는 `claude_oauth`/`chatgpt_oauth`처럼 풀링을 지원합니다 — `account` 대신 `accounts = [...]`를 사용해 저장된 여러 Kimi 계정으로 부하를 분산할 수 있습니다. 전체 안내는 [Kimi → Kimi Code (OAuth subscription)](/ko/providers/kimi/#kimi-code-oauth-subscription)를 참고하세요.

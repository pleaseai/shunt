...
shunt는 요청/응답 본문, 헤더, 자격 증명을 절대 내보내지 않습니다.

- **메트릭과 트레이스**는 저카디널리티이며 본문이 없습니다. 요청 스팬의 클라이언트 **세션 id**는 `include_session_id = true`(기본 꺼짐)일 때만, 그리고 트레이스 익스포트가 활성화된 동안에만 붙습니다.
...
내보내는 리소스는 `service.*`와 `telemetry.sdk.*`만 광고합니다 — host나 process detector가 실행되지 않아 머신 호스트네임은 붙지 않습니다 — 여기에 표준 `OTEL_RESOURCE_ATTRIBUTES`로 설정한 값이 더해집니다.

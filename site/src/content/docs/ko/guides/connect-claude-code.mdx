...
게이트웨이 base URL과 `apiKeyHelper`를 함께 담은 인라인 `--settings` 문서와 함께 Claude Code를 실행하며, 그 호출에만 적용됩니다 — 설정 파일은 그대로이고, 셸에 이미 export된 `ANTHROPIC_BASE_URL`보다 이 문서가 우선합니다(Claude Code 2.1.234 기준 실측). `claude` 뒤의 인자는 그대로 전달되며, `--config`는 조용히 삼켜지는 대신 거부되므로 shunt가 소유한 플래그는 `--` 뒤에 두세요. [`shunt gateway claude`](/ko/reference/cli/#shunt-gateway-claude)를 참고하세요.

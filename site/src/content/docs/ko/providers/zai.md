---
title: Z.ai (GLM)
description: ZAI_API_KEY로 매핑된 모델을 Z.ai의 Anthropic 호환 GLM 엔드포인트로 라우팅하기.
---

**Z.ai**는 자신의 **GLM** 모델을 **Anthropic 호환** 엔드포인트를 통해 제공합니다 — shunt는 Claude
Code의 Messages 요청을 그대로 전달하고 Z.ai API 키를 주입합니다. 내장 프리셋이 없으므로, 업스트림이
...
레거시 `[providers.zai]` 테이블 형식도 계속 지원됩니다 — 다만 한 파일에서 `[[upstreams]]`와
`[providers.*]`를 섞지 마세요.
...
`ZAI_API_KEY`가 설정되어 있지 않으면 `zai`로 라우팅된 첫 요청이 인증 오류를 반환합니다.
...
응답의 `x-gateway-upstream` 헤더가 `zai`를 가리키는지 확인한 다음,
[Claude Code를 shunt로 지정](/ko/guides/connect-claude-code/)하세요.

## 서브에이전트 플러그인

[`shunt-zai` 플러그인](https://github.com/pleaseai/shunt/tree/main/plugins/shunt-zai)은 위 모델마다

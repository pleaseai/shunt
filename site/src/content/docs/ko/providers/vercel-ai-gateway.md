...
description: AI_GATEWAY_API_KEY로 매핑된 모델을 Vercel AI Gateway의 Anthropic 호환 엔드포인트를 통해 라우팅하기.
---

**Vercel AI Gateway**는 여러 모델 벤더를 키 하나 뒤에 놓고 **Anthropic 호환** 엔드포인트를
노출합니다 — shunt는 Claude Code의 Messages 요청을 그대로 전달하고 게이트웨이 키를 주입합니다. 내장
프리셋이 없으므로, 업스트림이 `kind`와 `base_url`을 명시적으로 선언합니다.
...
이 게이트웨이는 bearer 인증(기본값)과 Anthropic의 `x-api-key` 헤더를 모두 받아들입니다 — 후자를
선호한다면 auth 맵에 `header = "x_api_key"`를 추가하세요. 레거시 `[providers.vercel]` 테이블 형식도
...
`AI_GATEWAY_API_KEY`가 설정되어 있지 않으면 `vercel`로 라우팅된 첫 요청이 인증 오류를 반환합니다.

## 모델

AI Gateway 모델 id는 `vendor/model` 슬러그입니다(예: `anthropic/claude-opus-4.8`) —
[AI Gateway 모델 카탈로그](https://vercel.com/ai-gateway/models)를 참고하고 도달 가능하게 하고 싶은
슬러그마다 `[[routes]]` 항목을 하나씩 추가하세요. Claude Code에서 라우팅된 id는 `ANTHROPIC_MODEL`,
...
응답의 `x-gateway-upstream` 헤더가 `vercel`을 가리키는지 확인한 다음,

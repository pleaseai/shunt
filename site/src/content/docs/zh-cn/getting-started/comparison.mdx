...
| tool-search / `defer_loading` / `tool_reference` 处理 | ◐(标准 OpenAI/Codex 上 gpt-5.4+ 默认原生⁸;其他情况仍为垫片) | ○⁵ | ◐(上游)/ ●(分叉) | ○ | ○ |
...
⁸ **[#82]** 增加了一个按提供方的 `tool_search` 标志(`src/config.rs:1136-1149,2792-2805`),把 Claude Code 的工具搜索映射到 OpenAI Responses API 自己的原生、客户端执行的 `tool_search` 协议 —— `ToolSearch` → `tool_search`,其 `tool_use` → `tool_search_call`,`tool_reference` → 一个以结构化 JSON 携带已加载工具完整 schema 的 `tool_search_output` 项(`src/model/responses_request.rs`) —— 而不是把 schema 折叠进文本。**默认开启(issue #286)**:垫片会在每次揭示时都使缓存的提示前缀失效(它必须把每个被揭示的工具重新加回 `tools` 数组,而 shunt 每一轮都会依据客户端历史重新推导这个数组),因此原生的、只追加 `input` 的路径现在在其适用之处 —— 路由到 gpt-5.4+ 模型的标准 OpenAI 或 ChatGPT/Codex Responses 风格 —— 成为默认行为。这些原生形状已针对 ChatGPT/Codex 后端在 gpt-5.6 上做过实时探测验证(2026-07-13, [#86]);`tool_search` 已被记录在公开的 OpenAI Responses API 文档中,适用于 gpt-5.4+ 模型。xAI/Grok 路由和 gpt-5.2 及以下的模型无论如何都保留 #43 垫片 —— 静默地,绝不报错。设置 `tool_search = false` 可在支持原生路径的提供方上改回垫片。
...
- **A. tool-search 上下文节省(已跟踪:[#43])。** **已基本由 [#82]/[#286] 解决**:一个默认开启的按提供方 `tool_search` 标志,现在为路由到 gpt-5.4+ 模型的标准 OpenAI 或 ChatGPT/Codex 提供方,把工具搜索映射到 Responses API 原生、客户端执行的 `tool_search` 协议,而不是文本垫片(见上文脚注 8)。剩余的缺口是不满足条件的风格/模型:xAI/Grok 路由和 gpt-5.2 及以下的模型仍会回退到 shim —— 它会把每个尚未加载的延迟工具排除在 `tools` 数组之外(`src/model/responses_request.rs:791-797`),所以对那些从未被揭示的工具,它确实收回了上下文 —— 但一旦 Claude Code 揭示了某个工具,shim 就会把它的完整 schema 同时渲染为 `tool_reference` 文本(`:623-643`)*并且*把该工具重新加入 `tools` 数组,于是每次揭示都会重新发送一遍 schema,并使此后的缓存提示前缀失效。Responses API 只允许模型调用它在 `tools` 中能看到的工具,因此在不使用原生协议的情况下,这种双重发送无法避免 —— 这个残余缺口只有等 xAI/Grok 以及更旧的 OpenAI 模型在上游各自获得等效的 `tool_search` 支持后才会消失。

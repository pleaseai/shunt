...
默认构建不需要 Node 工具链,也不包含管理仪表盘的前端包。若要构建内嵌管理 SPA 的
二进制,请先构建前端包,再启用 `ui` 特性。构建前端包需要 Node.js 22.12 或更高
版本:
...
上面提供的预构建发布二进制就是这样构建的,因此这一步只与从源码构建有关。该特性
新增了什么,参见[端点](/zh-cn/reference/endpoints/)。

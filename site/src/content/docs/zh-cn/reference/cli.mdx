...
已保存的 token 会一直使用到距离过期不足 5 分钟，然后才刷新。网关的 refresh token 会轮换且只能使用一次，因此刷新在独占文件锁下执行，并在拿到锁之后重新读取会话：多个 Claude Code 会话同时调用 helper 也只会发生一次刷新，不会因重放而使整个轮换家族失效。

锁和刷新都不会无限期等待。与网关的每次往返有上限，等待锁也有上限 —— 否则一个只接受连接却从不应答的部署会一直握着锁，让该机器上其他所有 `apiKeyHelper` 毫无提示地卡住。超出上限时命令会失败，并指出锁文件的路径。

进入 5 分钟缓冲期意味着**该刷新了**，而不是**必须刷新**。如果刷新失败而缓存的 token 仍然真正有效，shunt 会在 stderr 上给出警告并继续提供该 token，而不是把一次短暂的网络抖动变成认证失败。等它真正过期之后，命令才会失败。凡是 Claude Code 的 helper 校验器会拒绝的 token —— 不是 1~16384 个不含空白的可打印 ASCII 字符 —— 都会在这里被拒绝并给出指明网关的诊断，而不是被打印出去、导致认证失败却没有任何说明。`shunt gateway logout` 会连同会话一起删除锁文件。
...
完整的 upstream 配置见 [Kimi](/zh-cn/providers/kimi/#kimi-code-oauth-subscription)。

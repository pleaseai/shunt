...
| 映射模型上出现 `400 Invalid schema for function '…': '…' is not a 'regex'` | OpenAI 后端用 Python 的 `re` 编译工具 schema 中的每个 `pattern`,因此会拒绝仅 JavaScript 支持的正则(`\p{Cc}`、`(?<name>…)`、`\u{…}`);Claude Code 的 `Artifact` 工具就带有这样的 pattern。shunt 会从转发的 schema 中去掉这些 pattern(约束仍写在 `description` 里),若仍看到此错误请升级。 |

...
| `400 Invalid JSON schema: regex lookaround is not supported` | ChatGPT 的 schema 编译器也会拒绝 Python 接受的前瞻和后顾断言。此本地兼容性补丁从发送的工具 schema 中移除这些 pattern，包括 Neon `create_auth_user` 的邮件地址 pattern。工具执行和服务端输入验证保持不变。 |

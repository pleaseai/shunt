...
| 映射模型上的工具调用带着所有可选参数填充到达 | 后端会把没有显式 `strict` 的 function 工具向 strict 模式归一化,该模式下闭合的参数对象(`additionalProperties:false`)表现得像所有属性都是必填,于是模型会填充调用方从未设置的值。shunt 在转发的 function 工具和 `tool_search_output` 中回放的规格上固定 `strict:false`,让可选属性保持可选; xAI/Grok 路由上未验证故不发送该字段,原生 `tool_search` 的 schema 没有可选属性因此省略。若仍看到此现象请升级。 |
...
| Anthropic 适配器模型返回 429 | 检查网关日志中的 `rate_limit_kind`。`quota`(带有 `retry-after` / `anthropic-ratelimit-*` 头部)是真实的速率限制 —— 请退避或减少并行负载。`client-shape-rejection`(OAuth 请求、两种头部都没有、body 只有 `"Error"`)表示 api.anthropic.com 拒绝了一个不像 Claude Code 的订阅 OAuth 请求 —— 非 Claude Code 客户端必须使用 API 密钥而不是 OAuth token。Claude Code 的 auto 模式权限分类器是唯一省略上游所检查的 identity 块的 first-party 请求;shunt 仅对该请求恢复此块,因此 auto 模式不会再以“model temporarily unavailable”fail-closed。其余请求除 [Anthropic 提供方页面](/zh-cn/providers/anthropic/)所述的 thinking signature 移除之外,均保持 byte-for-byte 透传。若要让这些检查不落在昂贵的模型上,可用 [`classifier_model`](/zh-cn/providers/anthropic/#自动模式分类器) 固定它们。`no-ratelimit-headers`(非 OAuth 凭据)是缺少速率限制元数据的提供方 429 —— 按 `quota` 处理。 |
| Claude Code 提示本会话不符合 auto 模式免费分类器请求的条件 | auto 模式会请求 API 在服务端对每次工具使用进行分类,而只有 `api.anthropic.com` 会作出响应。在 Anthropic 路由上,shunt 原样中继该请求及其响应。由其他上游处理的路由(Codex/Responses、Cursor、Gemini、Antigravity)没有可中继的服务端判定,因此 shunt 不是什么都不返回,而是逐个动作回复“无法评估”:这样 Claude Code 只在本地分类那一个动作,并在下一轮继续询问服务端,而不会在本会话剩余时间内放弃服务端分类器、改用自己的计费分类器。对 Anthropic 协议的第三方(Kimi、OpenRouter、DeepSeek 等),在请求发出前还会剥离 `safeguards` 字段及其 `dangerous-tool-use-*` beta token:没有任何一家被确认接受该字段,而一旦返回点名该字段的 `400`,Claude Code 会在 `/clear` 之前拒绝 auto 模式的所有工具使用;剥离字段则只损失该轮的服务端判定。若要完全关闭该功能并让分类留在客户端,请设置 `CLAUDE_CODE_AUTO_MODE_SERVER=0`。 |

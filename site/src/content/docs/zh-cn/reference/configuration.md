...
| `tool_search` | `true`(默认) \| `false` | 在标准 OpenAI / ChatGPT-Codex 风格且模型为 GPT-5.4+ 时,为 Claude Code 的工具搜索使用原生的客户端执行 `tool_search` 协议;其他风格/模型无论如何都保留文本 shim。设为 `false` 可在支持原生路径的提供方上强制改回 shim。见 [Codex → 工具搜索](/zh-cn/guides/codex/#原生协议)。 |

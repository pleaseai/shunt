...
链路解析完成后，若请求需要该适配器已知无法提供的功能（tools、图片、structured output、显式 reasoning effort，或 `[1m]`/`[1M]` 上下文提示），后续条目仍可能被剔除。主路由始终保留。`[1m]`/`[1M]` 提示目前会排除所有回退，因为尚无适配器宣称已验证的 1M 路径。这些排除会记为 `capability_excluded`，不会被记为最佳失败。

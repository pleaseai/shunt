...
| `kind` | 无 preset 时 | `anthropic`、`responses`、`cursor`、`openai_chat`、`gemini`、`antigravity` 或 `antigravity_cli`。`openai_chat` 是有界的纯文本 Anthropic Messages → OpenAI Chat Completions 适配器，工具转换尚未支持并会被拒绝。后四者在有序 upstream 中必须显式设置 `kind`。 |
...
| 键 | 取值 | 含义 |
| :-- | :-- | :-- |
| `kind` | `anthropic` \| `responses` \| `cursor` \| `openai_chat` \| `gemini` \| `antigravity` \| `antigravity_cli` | 上游协议 / 适配器。`openai_chat` = 有界的纯文本 Anthropic Messages → OpenAI Chat Completions 转换（支持流式 usage trailer，工具转换暂不支持）;`anthropic` = Messages API;`responses` = 转换为 OpenAI Responses API;`cursor` = 原生 Cursor ConnectRPC/protobuf AgentService;`gemini` = Gemini Code Assist 转换;`antigravity` = Google Antigravity HTTP 后端;`antigravity_cli` = **已弃用** —— 运行本地 CLI。 |

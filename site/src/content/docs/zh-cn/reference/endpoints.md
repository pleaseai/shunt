...
| `GET` | `/routes` | shunt 原生路由发现 —— 逐字返回配置的 `[[routes]]` 表(model → provider/upstream_model/effort 映射,包括 claude 前缀的发现别名);区别于 `/v1/models`,后者提供更窄的 Anthropic 协议发现响应(`id`、`display_name` 以及上游模型元数据)。只要有一个 `[[models]]` 条目配置了[阶段路由器](/zh-cn/guides/stage-router/),响应就会多出一个 `routers` 数组,列出每个路由器的模型 id、两个档位的目标,以及 picker 出发的档位 |

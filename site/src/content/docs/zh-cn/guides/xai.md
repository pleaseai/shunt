...
编码/前沿 slug 有 `grok-4.5`、`grok-4.3` 和 `grok-build-0.1`;shunt 的模型
[发现](/zh-cn/guides/model-discovery/) 会透出 `grok-4.5` 和 `grok-4.3`。在路由中用 `upstream_model`
可将一个别名映射到一个实时 slug,而无需改动你的 Claude Code 环境。
...
同一节 [Codex 章节](/zh-cn/guides/codex/#4-select-the-model-in-claude-code) 逐字覆盖其余内容:
...
保持未设置。完整优先级和力度表:[力度与上下文](/zh-cn/guides/effort-and-context/#reasoning-effort)。
...
```bash
export CLAUDE_CODE_MAX_CONTEXT_TOKENS=<your-slug's real window>   # 按 xAI 的模型文档
```

它是**全局**的(每个会话一个值);请将其设置为你映射模型中最小的真实窗口,
因为超过某个模型的真实窗口会导致 `prompt is too long` 溢出反复折腾。详情和
`count_tokens` 行为见:[力度与上下文](/zh-cn/guides/effort-and-context/#context--usage-display-for-mapped-models)。

## 网页搜索

Claude Code 内置的**网页搜索在 Grok 路由上不生效。** xAI 的 Responses API 只接受
函数工具,因此 shunt 会在 `xai` 风格上(`grok` 和 `xai` 均如此)丢弃托管的 `web_search` 工具。
当你需要托管的网页搜索时,请使用 [`codex` 或 `openai` 路由](/zh-cn/guides/codex/#web-search)。
...
| 症状 | 原因 / 修复 |
| :-- | :-- |
| 启动时出现 `run shunt login xai` | 没有 `~/.shunt/xai-auth.json`(或 `$XAI_AUTH_FILE` 错误)。运行 `shunt login xai`。 |

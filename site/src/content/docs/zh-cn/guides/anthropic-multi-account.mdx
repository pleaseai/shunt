...
- 默认切换阈值是 `0.98`。当 5 小时或共享 `7d` status 为 `rejected` 时,该账户对所有模型都被视为接近配额。当 `7d_oi` status 为 `rejected` 时,该账户对 Fable 请求被视为接近配额。status 的拒绝判定不取决于对应使用率头部是否存在。计算使用率阈值和余量时,Fable 模型在 `7d_oi` 使用率存在时使用它,否则使用共享 `7d`。聚合 status 仅在没有记录任何按窗口 status 时作为兼容回退使用。阈值可以按账户(上文的 `threshold*` 字段)或池级(见[调优选择](#调优选择serverpool))调低。

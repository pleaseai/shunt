...
- 默认切换阈值是 `0.98`。当 5 小时 status 为 `rejected`、起决定作用的周 status 为 `rejected`、共享 5 小时使用率达到其阈值,或起决定作用的周使用率达到其阈值时,该账户即接近配额。对 Fable 模型,起决定作用的周 status 在 `7d_oi` 使用率存在时为 `7d_oi`,否则为共享 `7d`。聚合 status 仅在没有记录任何按窗口 status 时作为兼容回退使用。阈值可以按账户(上文的 `threshold*` 字段)或池级(见[调优选择](#调优选择serverpool))调低。
...
| :-- | :-- |
| 2xx | 中继并标记为健康。 |
| Fable 请求上仅 `anthropic-ratelimit-unified-7d_oi-status: rejected` 的 429 | Fable 配额耗尽:按数值 `retry-after` 施加仅对 Fable 请求生效的冷却(默认 60 秒,钳制到 1–3600 秒),然后轮换。非 Fable 请求仍可使用该账户。 |

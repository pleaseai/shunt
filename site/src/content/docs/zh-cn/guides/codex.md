...
当一个 Codex 池中有两个名称携带相同的 `account_id` 时,shunt 会将它们视为**同一个账户**——但前提是该 `account_id` 已被实际解析。由存储扫描发现的别名(`accounts` 列表为空)由扫描本身完成解析,因此会自动合并(coalesce)。通过 `credentials` 或 `token_env` 显式配置的条目只在 selection 之后按请求解析其 id,因此只有在你显式设置匹配的 `uuid` 时,才会与其他别名合并。合并后的别名共享冷却、健康状态和刷新锁,故障转移会跳过重复的身份而不是重试它。`priority` 最低的已启用别名(其次是第一个条目)提供唯一会被尝试的 token,shunt 会记录一条重复身份警告。存储发现会读取 `tokens.account_id` 或 JWT claim,并按目录 mtime 缓存扫描结果。详情见 [Codex 多账户指南](/zh-cn/guides/codex-multi-account/)。
:::

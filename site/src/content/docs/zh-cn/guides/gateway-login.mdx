...
已弃用的 `jwt_secret_env`(env 变量名,默认 `SHUNT_GATEWAY_JWT_SECRET`)和 `token_ttl_seconds`(默认 `3600`)在单独使用时仍完全受支持,`token_ttl_seconds` 仍是设置小于一小时生命周期的唯一方式。同时设置一个已弃用的键和它对应的 `session.*` 替代键会导致启动失败(`jwt_secret_env` 与 `session.jwt_secret`,或 `token_ttl_seconds` 与 `session.ttl_hours`)。只有当配置文件中显式设置了这两个键之一时,shunt 才会记录一次弃用警告。完整的优先级规则和密钥轮换步骤见[配置参考](/zh-cn/reference/configuration/#servergatewaysession可选)。

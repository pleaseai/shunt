...
网关登录会话带有 Claude apps 网关的功能取舍:WebSearch 被禁用,仅限第一方的 beta 头部和一小时 cache-TTL beta 会被省略,而且登录需要浏览器。对于个人或单用户的部署,请继续使用 [`ANTHROPIC_BASE_URL`](/zh-cn/guides/connect-claude-code/) 以及可选的 [`[server.auth]`](/zh-cn/guides/shared-gateway/)。
...
添加 Claude Code 和用户浏览器可以触达的公开 URL:
...
如果 `public_url` 不是一个纯 HTTPS origin(只有回环地址允许 `http`)、`token_ttl_seconds` 为零、签名密钥短于 32 字节,或者既没有配置有效的用户列表也没有配置有效的外部 IdP,启动会安全失败(fail closed)。静态用户的 secret 中可以包含 `:`,因为只有第一个冒号用于分隔 email 与其 secret。
...
非空的遥测目标列表会下发六个标准的 Claude Code OTLP 环境变量值。策略中的 `env` 键覆盖注入的默认值:
...
目前这项配置只门控托管环境变量的下发。需认证的 OTLP 接收/中继路由将在 M-C(#189)中单独提供。
...
托管策略、`ETag` 缓存、遥测环境变量下发以及服务端模型允许列表的强制执行已在上文描述。需认证的入站 OTLP 遥测仍是单独的 M-C 后续工作。

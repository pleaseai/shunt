...
`antigravity-cli` 这条路径按每个请求以 agent 模式启动一个 `agy` CLI 子进程。`agy` 自己解析自己的工具调用,因此请求中的
`tools` 数组不会被转发,响应里也永远不会包含 `tool_use` 块。需要自行驱动工具循环的客户端应改用 `gemini` 提供方。
...
建议优先使用上面那个原生的 `antigravity` HTTP 提供方,它完全不需要这些。完整细节见
[已废弃的 antigravity-cli 传输](/guides/providers/#the-deprecated-antigravity-cli-transport)。
...
| [Kimi Code(订阅制,OAuth)](/zh-cn/providers/kimi/#kimi-code-oauth-subscription) | `https://api.kimi.com/coding` | 使用你的订阅暴露的模型 ID |
...
[Kimi → Kimi Code (OAuth subscription)](/zh-cn/providers/kimi/#kimi-code-oauth-subscription)。

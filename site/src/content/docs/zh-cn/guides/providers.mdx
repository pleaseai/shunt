...
第三个设置 `profile_dir` 为每个条目提供独立的 `HOME`。`agy` 通过 `HOME` 解析包括凭据在内的整个状态树，因此不设置时
所有条目都共享网关主机上唯一的 ambient `~/.gemini` 配置，最终只能服务一个 Google 账号。给每个条目指定各自的目录后，
它们会分别登录自己的账号，并可以同时池化多个账号。该目录按需创建并规范化为绝对路径，开头的 `~` 会被展开。每个目录都
需要各自执行一次 `agy` 登录。仅对使用了该设置的条目，子进程的 `GOOGLE_*` / `GEMINI_*` 环境变量会被剥离，这样主机自身
的配置就无法悄悄改变由哪个账号 —— 以及谁的账单 —— 来处理请求。不设置（默认）则与既有行为完全一致。有序
`[[upstreams]]` 条目同样支持该设置。
...
ChatGPT 账户的 Codex 后端**拒绝** `gpt-*-codex` slug —— 它只接受该账户实时授权的 slug。权威目录是 openai/codex 的 [`models.json`](https://github.com/openai/codex/blob/main/codex-rs/models-manager/models.json)。当前的 slug 有 `gpt-6-astra`、`gpt-6-sol`、`gpt-6-luna`(最新)、`gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna`(前沿)以及 `gpt-5.5` / `gpt-5.4`;较老的账户可能只被授权使用较早的那些。在路由中使用 `upstream_model` 可将任意别名映射到一个已授权的 slug。
...
| [OpenCode Zen](/zh-cn/providers/opencode/) | `https://opencode.ai/zen` | `claude-fable-5-1`, `gpt-6-astra` — 精选跨厂商目录(读取 `x-api-key`) |
...
| :-- | :-- | :-- |
| `shunt-codex` | `gpt-6-sol`、`gpt-6-luna`、`gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna` | `codex`(ChatGPT 订阅) |

...
在被门控的路由上,被接受的凭据头部在转发前总会被剥除(shunt 会在那里注入它自己的提供方凭据),匹配是常量时间的,token 值从不被记录(客户端*名称*会,按请求)。在透传路由上 shunt 不注入任何东西,因此调用方自己的凭据会按呈现的样子转发 —— 但按槽位有例外:当 `Authorization` 或 `x-api-key` 槽位自身的值是 shunt 自己的入站凭据(被接受的客户端 token,或 `[server.gateway]` JWT)时,该槽位会被清除。因此在门控处被接受的凭据绝不会被中继到上游,而另一个槽位中真实的上游凭据仍会继续流动。
...
- **混有透传模型时**:每个调用方真实的 Anthropic 凭据需要一个自己的槽位,因此请通过配置的头部分发专用 token,并把 `Authorization` / `x-api-key` 留给它(`ANTHROPIC_CUSTOM_HEADERS` 每行接受一个 `Name: Value`)。这是卫生做法,而非防止泄露的唯一手段:确实出现在 `Authorization` 或 `x-api-key` 中的客户端 token,会在任何透传转发之前按值从该槽位清除。

...
没有运维人员介入就无法通过任何重试恢复的账户,会显示为 **Needs re-login** 而不是 `cooling` —— 主表格和高级池表格都是如此。这个区分很重要:冷却会在 5 分钟后自行到期并重新尝试该账户,因此永久失效的凭据否则会**永远**看起来和配额暂停一模一样。该标记在以下情况置位:提供方终结性拒绝了 refresh 授权、根本无法刷新的凭据(`token_env` 令牌或已过期的 setup token)被拒绝,以及用刚刷新的 bearer 重试的请求**仍然**被拒绝。它与冷却相互独立上报,且仅存于内存,因此重启后会清空,直到该账户再次失败。

...
它在启动 Claude Code 时附带一个同时包含网关 base URL 和 `apiKeyHelper` 的内联 `--settings` 文档，只作用于这次调用 —— 你的设置文件不受影响，而且该文档的优先级高于 shell 中已经 export 的 `ANTHROPIC_BASE_URL`（针对 Claude Code 2.1.234 实测）。`claude` 之后的参数会原样转发；`--config` 会被拒绝而不是被悄悄吞掉，因此 shunt 自己的标志请放在 `--` 之后。参见 [`shunt gateway claude`](/zh-cn/reference/cli/#shunt-gateway-claude)。

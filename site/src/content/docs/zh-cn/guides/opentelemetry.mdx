...
| `shunt.router.decisions` | 计数器 | `model`, `algorithm`, `target`, `source` | 所有 [`[models.router]`](/zh-cn/reference/configuration/) `type` 的路由决策，按路由器的模型 id（匹配后的形式，`[1m]` 提示已剥离）、配置的 `type`、该决策解析到的模型 id 以及选中的原因计数。不含 `count_tokens` 探测。 |

...
本页讲的是 shunt **自身**向外发送的遥测。网关独立的[遥测接收](/zh-cn/guides/gateway-login/)处理相反的方向 —— 托管 Claude Code **客户端**发送*给* shunt 的 OTLP 载荷,verbatim 中继到 `[server.gateway.telemetry]` 中的目标。两者独立配置,可以指向不同的 collector。
...
| 序列 | 类型 | 属性 | 含义 |
| :-- | :-- | :-- | :-- |
| `shunt.requests` | 计数器 | `provider`, `model`, `http.response.status_code`, `caller` | 代理的推理请求。`caller` 对入站流量为 `client`，对 [`[models.router]`](/zh-cn/reference/configuration/) 条目代表调用方发起的裁判调用为 `router`。 |
| `shunt.latency` | 直方图(ms) | `provider`, `model`, `http.response.status_code`, `caller` | 流式响应为到 header 的延迟；其他响应为完整延迟。提交式流式响应因 header 在上游响应前提交，测量从分发到尝试分类的时间。 |
| `shunt.ttft` | 直方图(ms) | `provider`, `model` | 从请求开始到第一个完整的非保活 SSE 帧的时间。 |
...
| `shunt.gateway_telemetry_ingest` | 计数器 | `signal`, `outcome` | 按接收结果统计入站网关 OTLP 载荷（`metrics`/`logs`/`traces`）：`relayed`；没有任何目标 opt-in 时为 `discarded`；在途中继上限饱和时为 `shed`；bearer 无效或 body 超限时为 `rejected`。 |
...
| `shunt.router.decisions` | 计数器 | `model`, `algorithm`, `target`, `source` | 所有 [`[models.router]`](/zh-cn/reference/configuration/) `type` 的路由决策，以及 [`[models.subagents]`](/zh-cn/reference/configuration/#modelssubagents可选) 覆盖层的决策（`algorithm = "subagents"`），按作出该决策的 `[[models]]` 条目 id（匹配后的形式，`[1m]` 提示已剥离）、配置的 `type`、该决策解析到的模型 id 以及选中的原因计数。不含 `count_tokens` 探测。 |
| `shunt.stage_router.decisions` | 计数器 | `model`, `tier`, `source` | [阶段路由器](/zh-cn/guides/stage-router/)的档位决策，按路由器的模型 id（匹配后的形式，`[1m]` 提示已剥离）、选中的档位（`capable`/`efficient`）以及选中的原因计数。不含 `count_tokens` 探测。 |
| `shunt.router.judge_calls` | 计数器 | `model`, `algorithm`, `outcome` | 配置了裁判的 [`[models.router]`](/zh-cn/reference/configuration/#modelsrouterclassifier可选) 条目发起的裁判调用，按路由器的模型 id、配置的 `type` 以及该次调用的结果计数：`decided`，或 `timeout`、`oversized`、`upstream_error`、`invalid_reply`、`translation_failed`、`budget_exhausted` 之一。`budget_exhausted` 表示会话已用完 `max_judge_calls`，调用被拒绝。在阶段路由器上，后几种结果都会把该轮路由到 picker 的默认值；在 `llm_classifier` 或 `composite` 条目以及 classifier 形式的 `[models.subagents]` 覆盖层上，由算法自己的回退路径收尾。未发起调用、按已保留分配应答的回合记为 `retained`。在 [`escalation` 或 `advisor`](/zh-cn/reference/configuration/#被扣住的回合escalation-与-advisor) 条目上，完成的回合之后发生的失败会改为提供被扣住的那一轮，advisor 的审阅计入 `algorithm = "advisor"`；被扣住的回合本身不算裁判调用。 |
| `shunt.stage_router.flips` | 计数器 | `model`, `from`, `to` | 把会话从已固定档位移开的决策。升档刻意比降档容易，因此两个方向分开计数。 |
| `shunt.requests_shed` | 计数器 | — | 在 `max_concurrent_requests` 上限处被拒绝的入站请求。拒绝本身仅以 `debug!` 记录，因此这个计数器才是网关饱和的可见信号。 |
...
| `shunt.pool.reprobes` | 计数器 | `provider` | 在陈旧近配额 Codex/ChatGPT 账户的首次 HTTP 发送时提交的重新探测；启用 WebSocket 的提供方只统计 inbound HTTP 探测。 |
| `shunt.upstream.status` | 仪表 | `provider` | 观测到的 [`[server.status]`](/reference/configuration/#serverstatus-optional) Statuspage severity：`0` none、`1` minor、`2` major、`3` critical。当前处于 `unknown`（无信号）状态的 provider 不会以 `0` 上报，而是从该序列中完全省略。 |

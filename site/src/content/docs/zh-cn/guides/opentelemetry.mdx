...
| `shunt.router.judge_calls` | 计数器 | `model`, `algorithm`, `outcome` | 配置了裁判的 [`[models.router]`](/zh-cn/reference/configuration/#modelsrouterclassifier可选) 条目发起的裁判调用，按路由器的模型 id、配置的 `type` 以及该次调用的结果计数：`decided`，或 `timeout`、`oversized`、`upstream_error`、`invalid_reply`、`translation_failed`、`budget_exhausted` 之一 —— 后几种都会把该轮路由到 picker 的默认值。 |

...
| `shunt.stage_router.decisions` | 计数器 | `model`, `tier`, `source` | [阶段路由器](/zh-cn/guides/stage-router/)的档位决策，按请求的模型 id、选中的档位（`capable`/`efficient`）以及选中的原因计数。不含 `count_tokens` 探测。 |

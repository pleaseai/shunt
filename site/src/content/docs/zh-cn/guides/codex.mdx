...
启用后,shunt 默认把工具搜索映射到 OpenAI Responses API 自身的**原生、客户端执行的 `tool_search`**
协议:Claude Code 的 `ToolSearch` 工具变为 `tool_search`(`execution: "client"`)工具,其 `tool_use`
变为 `tool_search_call`,而 `tool_reference` 结果变为一个 `tool_search_output` 条目,以结构化 JSON
携带已加载工具的完整 schema,附加在 `input` 的末尾。这一点很重要,因为另一种做法 —— 把被揭示工具的
schema 折回请求的 `tools` 数组 —— 会在每次揭示时使缓存的提示前缀失效:`tools` 正是这个可缓存前缀的
一部分,shunt 每一轮都会依据客户端历史重新推导它,数组一旦增长,就会迫使后端从工具边界开始重新处理整段
对话。原生协议只以追加方式扩展 `input` 历史,`tools` 保持不变,因此缓存前缀在每次揭示后都能存活。

- 默认行为无需改动 `shunt.toml` —— `ENABLE_TOOL_SEARCH` 这一层纯粹是 Claude Code 的环境变量。
- 适用于 `codex`(ChatGPT)与 `openai`(标准 Responses)提供方。
...
`tool_search` 在 `[providers.<name>]` 下默认是 `true`。这些原生形状已针对 ChatGPT/Codex 后端在
gpt-5.6 上做过实时探测验证(2026-07-13,[pleaseai/shunt#86](https://github.com/pleaseai/shunt/pull/86)),而且 `tool_search` 已被记录在公开的 OpenAI Responses API
文档中,适用于 gpt-5.4+ 模型。如果某个确实支持原生路径的提供方想改回文本 shim,设置
`tool_search = false`:
...
| 工具搜索不收回上下文(仍在为 shim 的缓存失效代价买单) | 检查风格/模型门控 —— 原生 `tool_search` 需要标准 OpenAI/ChatGPT-Codex 风格和 gpt-5.4+ 模型;不受支持的组合会静默回退到 shim。若两项门控都满足,确认你没有把 `tool_search` 设为 `false`;见上文 [工具搜索 → 原生协议](#原生协议)。 |

...
当前 slug:`gpt-6-astra`、`gpt-6-sol`、`gpt-6-luna`(最新)、`gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna`(前沿)以及
`gpt-5.5` / `gpt-5.4`。较老的账户可能只被授权使用
...
一些 slug 带有 `minimal_client_version`(例如 `gpt-6-astra` 需要 ≥ 0.153.0,`gpt-6-sol` 和 `gpt-6-luna` 需要 ≥ 0.155.0)。当
请求的客户端身份缺失或过旧时,后端会应答 `Model not found <slug>`。
shunt 通过发送固定的 Codex CLI 身份头(`originator: codex_cli_rs`、
`user-agent`、`version`)来避免这一点,固定到 **openai/codex rust-v0.156.0**。见
...
提供了面向 `gpt-6-sol` / `-luna` 和 `gpt-5.6-sol` / `-terra` / `-luna` 的子 agent —— 在 `/plugin marketplace add pleaseai/shunt`
...
- 路由到的模型必须是 **gpt-5.4 及以上**(`gpt-5.4`、`gpt-5.5`,或 `gpt-5.6` 系列)。精确 slug
  `gpt-6-astra`、`gpt-6-sol`、`gpt-6-luna` 也符合条件。更早的 slug (`gpt-5.2` 及以下)无论 `tool_search` 如何设置都会保留 shim。

...
**发现过滤器。** Claude Desktop 的自动发现只显示*可识别为 Claude* 的 id —— 即 tier 命名的 id（`claude-sonnet-*`、`claude-opus-*`、`claude-haiku-*`、`claude-fable-*`）。shunt 的内置目录是 Anthropic API 与参考 Claude apps gateway 所提供列表的超集 —— 其中有十三个 tier 命名的 id，因此 Claude Desktop 会显示全部模型。这是回退用的快照;当 shunt 能够访问你的 Anthropic 上游时,会改为提供该调用方自己的实际列表(参见[模型发现](/zh-cn/guides/model-discovery/))。

```json
// GET /v1/models — 内置目录（auto_include_builtin_models），全部采用 tier 命名
// （每个条目还带有 "type": "model"、"display_name"、"created_at"、
//   "max_input_tokens" 和 "max_tokens"）
{ "data": [

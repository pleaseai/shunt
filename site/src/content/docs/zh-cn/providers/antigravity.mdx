...
为了不必猜测是哪种形态,shunt 会在请求路径上读取账号自己的目录(`fetchAvailableModels`)—— 按后端
...
配置中未识别的 `effort` 会按规则 1 所述原样追加,不会被收敛;但在 `-tiered` 路径上它会折叠为
`medium`,因为后端会拒绝它不认识的 `thinkingLevel`。已经以档位或 `-tiered` 结尾的 id、目录原样发布的
